...
{
  if (protocol != NDPI_OLD_RESULT_BASE_SSL) {
    ...
    if(packet->tcp != NULL) {
      switch(protocol) {
      case NDPI_OLD_RESULT_BASE_SSL:
      case NDPI_OLD_RESULT_BASE_SSL_NO_CERT:
	{
	  /* 
	  ...
	  */
	  ...
	  if((sport == 465) || (dport == 465))      protocol = NDPI_OLD_RESULT_APP_MAIL_SMTPS;
	  else if((sport == 993) || (dport == 993)) protocol = NDPI_OLD_RESULT_APP_MAIL_IMAPS;
	  else if((sport == 995) || (dport == 995)) protocol = NDPI_OLD_RESULT_APP_MAIL_POPS;
	}
	break;
      }
    }
...
int sslDetectProtocolFromCertificate(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
  ...
  if((packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_UNKNOWN)
    ...
    if(rc > 0) {
      ...
      printf("***** [SSL] %s\n", certificate);
#endif
      if(ndpi_match_string_subprotocol(ndpi_struct, flow, certificate, strlen(certificate)) != NDPI_OLD_RESULT_UNKNOWN)
    ...
    if((packet->ssl_certificate_num_checks >= 2)
      ...
      ndpi_int_ssl_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_BASE_SSL);
  }
...
{
#if defined(NDPI_PROTOCOL_SOFTETHER) || defined(NDPI_OLD_RESULT_APP_MEEBO)|| defined(NDPI_OLD_RESULT_APP_TOR) || defined(NDPI_PROTOCOL_VPN_X) || defined(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER) || defined (NDPI_PROTOCOL_OOVOO) || defined (NDPI_PROTOCOL_ISKOOT) || defined (NDPI_OLD_RESULT_APP_OSCAR) || defined (NDPI_PROTOCOL_ITUNES) || defined (NDPI_OLD_RESULT_SERVICE_GMAIL)
  ...
  u_int32_t a;
  u_int32_t end;
#if defined(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER)
  if (NDPI_COMPARE_PROTOCOL_TO_BITMASK(ndpi_struct->detection_bitmask, NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER) != 0)
    goto check_for_ssl_payload;
#endif
#if defined(NDPI_OLD_RESULT_APP_OSCAR)
  if (NDPI_COMPARE_PROTOCOL_TO_BITMASK(ndpi_struct->detection_bitmask, NDPI_OLD_RESULT_APP_OSCAR) != 0)
    goto check_for_ssl_payload;
#endif
 ...
 check_for_ssl_payload:
  ...
  for (a = 5; a < end; a++) {
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG, "ssl jabber packet match\n");
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
	    (ndpi_struct->detection_bitmask, NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER) != 0) {
	  ndpi_int_ssl_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER);
	  return;
	}
      }
    }
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_OSCAR, ndpi_struct, NDPI_LOG_DEBUG, "OSCAR SERVER SSL DETECTED\n");
	...
	ndpi_int_ssl_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_OSCAR);
	return;
      }
    }
...
#endif
  if(packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_UNKNOWN) {
    NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "found ssl connection.\n");
    ...
    if(!packet->ssl_certificate_detected
      ...
      ndpi_int_ssl_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_BASE_SSL_NO_CERT);
    } else
      ndpi_int_ssl_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_BASE_SSL);
  }
}
...
static u_int8_t ndpi_search_sslv3_direction1(struct ndpi_detection_module_struct *ndpi_struct, 
  ...
  if ((packet->payload_packet_len >= 5)
    ...
    u_int32_t temp;
    NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "search sslv3\n");
    // SSLv3 Record
    if (packet->payload_packet_len >= 1300) {
      return 1;
    }
    temp = ntohs(get_u_int16_t(packet->payload, 3)) + 5;
    NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "temp = %u.\n", temp);
    ...
    if (packet->payload_packet_len < temp && temp < 5000 && packet->payload_packet_len > 9) {
      ...
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG,
      ...
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "suspected start of certificate: %u\n",
	       cert_start);
...
void ndpi_search_ssl_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_BASE_SSL) {
  ...
  NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "search ssl\n");

  {
    ...
    if((packet->payload_packet_len > 5)
	   ...
	   && (packet->payload[3] >= 0)
	   && (packet->payload[3] <= 9))) {
      ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_SERVICE_WHATSAPP, NDPI_REAL_PROTOCOL);
      return;
    } else {
  ...
  if (packet->payload_packet_len > 40 && flow->l4.tcp.ssl_stage == 0) {
    NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "first ssl packet\n");
    // SSLv2 Record
	...
	&& (packet->payload_packet_len - packet->payload[1] == 2)) {
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "sslv2 len match\n");
	...
	&& (packet->payload_packet_len - ntohs(get_u_int16_t(packet->payload, 3)) == 5)) {
      // SSLv3 Record
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "sslv3 len match\n");
      flow->l4.tcp.ssl_stage = 1 + packet->packet_direction;
      return;
    }
  }
  ...
  if (packet->payload_packet_len > 40 && flow->l4.tcp.ssl_stage == 2 - packet->packet_direction) {
    NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "second ssl packet\n");
    // SSLv2 Record
    if (packet->payload[2] == 0x01 && packet->payload[3] == 0x03
	&& (packet->payload[4] == 0x00 || packet->payload[4] == 0x01 || packet->payload[4] == 0x02)
	&& (packet->payload_packet_len - 2) >= packet->payload[1]) {
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "sslv2 server len match\n");
      ssl_mark_and_payload_search_for_other_protocols(ndpi_struct, flow);
      return;
    }
    ...
    if (ret == 1) {
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "sslv3 server len match\n");
      ssl_mark_and_payload_search_for_other_protocols(ndpi_struct, flow);
      return;
    } else if (ret == 2) {
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG,
    ...
    if (packet->payload_packet_len > 40 && flow->packet_direction_counter[packet->packet_direction] < 5) {
      NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "need next packet\n");
      return;
    }
  }

  NDPI_LOG(NDPI_OLD_RESULT_BASE_SSL, ndpi_struct, NDPI_LOG_DEBUG, "exclude ssl\n");
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_BASE_SSL);
  return;
}
#endif
