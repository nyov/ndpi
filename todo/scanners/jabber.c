...
{
#if defined( NDPI_PROTOCOL_TANGO ) || defined( NDPI_OLD_RESULT_APP_TRUPHONE ) || defined( NDPI_OLD_RESULT_SERVICE_WHATSAPP )
  ...
  if (packet->payload_packet_len > x + 18 && packet->payload_packet_len > x && packet->payload_packet_len > 18) {
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_TRACE, "changed to TRUPHONE.\n");

	ndpi_int_jabber_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_TRUPHONE, NDPI_CORRELATED_PROTOCOL);
      }
    }
  }
#endif
...
void ndpi_search_jabber_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->tcp != NULL && packet->tcp->syn != 0 && packet->payload_packet_len == 0) {
    NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG, "check jabber syn\n");
    if (src != NULL && src->jabber_file_transfer_port[0] != 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG,
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct,
	...
	ndpi_int_jabber_add_connection(ndpi_struct, flow,
				       NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, NDPI_CORRELATED_PROTOCOL);
      }
    }
  ...
  if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER) {
    u_int16_t lastlen;
    u_int16_t j_port = 0;
    /* check for google jabber voip connections ... */
    /* need big packet */
    if (packet->payload_packet_len < 100) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG, "packet too small, return.\n");
      return;
    }
    /* need message to or type for file-transfer */
    if (memcmp(packet->payload, "<iq from=\"", 8) == 0 || memcmp(packet->payload, "<iq from=\'", 8) == 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG, "JABBER <iq from=\".\n");
      lastlen = packet->payload_packet_len - 11;
      for (x = 10; x < lastlen; x++) {
	if (packet->payload[x] == 'p') {
	  if (memcmp(&packet->payload[x], "port=", 5) == 0) {
	    NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG, "port=\n");
	    if (src != NULL) {
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG, "JABBER <iq to=\"/type=\"\n");
	...
	if (packet->payload[x] == '@') {
	  NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_DEBUG, "JABBER @\n");
	  break;
	}
      }
      if (x >= lastlen) {
  ...
  if ((packet->payload_packet_len > 13 && memcmp(packet->payload, "<?xml version=", 14) == 0)
	...
	if (memcmp
	  ...
	  NDPI_LOG(NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, ndpi_struct, NDPI_LOG_TRACE, "found JABBER.\n");
	  x += 47;

	  ndpi_int_jabber_add_connection(ndpi_struct, flow,
					 NDPI_OLD_RESULT_APP_UNENCRYPED_JABBER, NDPI_REAL_PROTOCOL);
...
#ifdef NDPI_OLD_RESULT_APP_TRUPHONE
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_TRUPHONE);
#endif
}
