...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_IPP, protocol_type);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "search ipp\n");
	if (packet->payload_packet_len > 20) {

		NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG,
		...
		if (packet->payload[i] < '0' || packet->payload[i] > '9') {
			NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "payload does not begin with a number.\n");
		...
		if (packet->payload[i++] != ' ') {
			NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "there is no blank following the number.\n");
			goto search_for_next_pattern;
		}

		if (packet->payload[i] < '0' || packet->payload[i] > '9') {
			NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "no number following the blank.\n");
			goto search_for_next_pattern;
		}
		...
		if (memcmp(&packet->payload[i], " ipp://", 7) != 0) {
			NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "the string ' ipp://' does not follow.\n");
			goto search_for_next_pattern;
		}

		NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "found ipp\n");
  ...
  search_for_next_pattern:
	...
	if (packet->payload_packet_len > 3 && memcmp(packet->payload, "POST", 4) == 0) {
		...
		if (packet->content_line.ptr != NULL && packet->content_line.len > 14
			...
			NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "found ipp via POST ... application/ipp.\n");
			ndpi_int_ipp_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
			return;
		}
	}
	NDPI_LOG(NDPI_OLD_RESULT_APP_IPP, ndpi_struct, NDPI_LOG_DEBUG, "no ipp detected.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_IPP);
}
