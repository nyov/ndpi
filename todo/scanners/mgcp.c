...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_MGCP, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	while ((pos + 5) < packet->payload_packet_len) {
		if (memcmp(&packet->payload[pos], "MGCP ", 5) == 0) {
			NDPI_LOG(NDPI_OLD_RESULT_APP_MGCP, ndpi_struct, NDPI_LOG_DEBUG, "MGCP match.\n");
  ...
  mgcp_excluded:
	NDPI_LOG(NDPI_OLD_RESULT_APP_MGCP, ndpi_struct, NDPI_LOG_DEBUG, "exclude MGCP.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_MGCP);
}
