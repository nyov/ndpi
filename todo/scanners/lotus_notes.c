				   ...
				   struct ndpi_flow_struct *flow)
{
  ...
  printf("[lotus_notes_packet_id: %u][len=%u][%02X %02X %02X %02X %02X %02X %02X %02X]\n", 
	...
	if(memcmp(&packet->payload[6], lotus_notes_header, sizeof(lotus_notes_header)) == 0) {
	  NDPI_LOG(NDPI_OLD_RESULT_APP_LOTUS_NOTES, ndpi_struct, NDPI_LOG_DEBUG, "Found lotus_notes.\n");
	  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_LOTUS_NOTES, NDPI_REAL_PROTOCOL);
	}

	return;
      }

      NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_LOTUS_NOTES);
...
void ndpi_search_lotus_notes(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_LOTUS_NOTES, ndpi_struct, NDPI_LOG_DEBUG, "lotus_notes detection...\n");

  /* skip marked packets */
  if(packet->detected_protocol_stack[0] != NDPI_OLD_RESULT_APP_LOTUS_NOTES)
