...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_MSN, protocol_type);
}
...
static void ndpi_search_msn_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "search msn tcp.\n");
#ifdef NDPI_OLD_RESULT_BASE_SSL
  if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_BASE_SSL) {
    NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "msn ssl ft test\n");
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "detected MSN File Transfer, ifdef ssl.\n");
    ...
    if (flow->packet_counter >= 5 && flow->packet_counter <= 10 && (get_u_int32_t(packet->payload, 0) == htonl(0x18000000)
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE,
  ...
  if (flow->packet_counter == 1
#ifdef NDPI_OLD_RESULT_BASE_SSL
      || ((packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_BASE_SSL) && flow->packet_counter <= 3)
#endif
      ) {
	...
	if (packet->content_line.ptr != NULL
	  ...
	  if ((src != NULL
	       && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_OLD_RESULT_APP_MSN)
	       != 0) || (dst != NULL
	  ...
	  for (a = 0; a < packet->parsed_lines; a++) {
		...
		(memcmp(packet->line[a].ptr, "CVR ", 4) == 0
		 || memcmp(packet->line[a].ptr, "VER ",
	      ...
	      NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct,
	...
	if (ndpi_int_find_xmsn(ndpi_struct, flow) == 1) {
	  NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "HTTP/1.0 200 OK .... X-MSN.\n");
	  ndpi_int_msn_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
	  return;
	}
      }
	...
	while (plen < endlen) {
	  if (ndpi_check_for_email_address(ndpi_struct, flow, plen) != 0) {
	    NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "found mail address\n");
	    break;
	  }
  ...
  if ((src != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_OLD_RESULT_APP_MSN) != 0)
      || (dst != NULL
	  && NDPI_COMPARE_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask, NDPI_OLD_RESULT_APP_MSN) != 0)) {
    if (flow->packet_counter == 1 &&
	packet->payload_packet_len > 12 && memcmp(packet->payload, "recipientid=", 12) == 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_DEBUG, "detected file transfer.\n");
      ndpi_int_msn_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
  }
  ...
  if (flow->l4.tcp.msn_stage == 0) {
	...
	&& get_u_int32_t(packet->payload, 0) == htonl(0x04000000)) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_DEBUG, "maybe first TCP MSN detected\n");
    ...
    if (packet->payload_packet_len > 10 && get_u_int32_t(packet->payload, 0) == htonl(0x666f6f00)) {
      ndpi_int_msn_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "MSN File Transfer detected 1\n");
      return;
    }
    /* did not see this pattern in any trace */
    if (packet->payload_packet_len == 56 && get_u_int32_t(packet->payload, 16) == 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_DEBUG, "maybe Second TCP MSN detected\n");
    ...
    ndpi_int_msn_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
    NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "MSN File Transfer detected 2\n");
    return;
	...
	(get_u_int32_t(packet->payload, 0) == htonl(0x10000000) || get_u_int32_t(packet->payload, 0) == htonl(0x30000000))) {
      ndpi_int_msn_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "MSN File Transfer detected 3\n");
      return;
    }
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "msn 7.\n");
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "exclude msn.\n");
 ndpi_msn_exclude:
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_MSN);
}
				     ...
				     *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->payload_packet_len == 20
    ...
    NDPI_LOG(NDPI_OLD_RESULT_APP_MSN, ndpi_struct, NDPI_LOG_TRACE, "msn udp misc data connection detected\n");
    ndpi_int_msn_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
  }
...
void ndpi_search_msn(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (NDPI_COMPARE_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_MSN) == 0) {
...
#if defined(NDPI_OLD_RESULT_BASE_HTTP)
	  || packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_BASE_HTTP
#endif
#if defined(NDPI_OLD_RESULT_BASE_SSL)
	  || packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_BASE_SSL
#endif
#if defined(NDPI_OLD_RESULT_APP_STUN)
	  || packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_STUN
#endif
	  ) {
