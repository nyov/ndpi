...
void ndpi_search_ayiya(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if(packet->udp && (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_UNKNOWN)) {
    u_int8_t i;
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_AYIYA, NDPI_REAL_PROTOCOL);

      return;
    }

    NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_AYIYA);
  }
}
#endif
