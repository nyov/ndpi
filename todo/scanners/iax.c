...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_IAX, NDPI_REAL_PROTOCOL);
}
...
static void ndpi_search_setup_iax(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (						/* 1. iax is udp based, port 4569 */
    ...
    if (packet->payload_packet_len == 12) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_IAX, ndpi_struct, NDPI_LOG_DEBUG, "found IAX.\n");
  ...
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_IAX);
