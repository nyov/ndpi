...
static void ndpi_check_gtp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if((packet->udp != NULL) && (payload_len > sizeof(struct gtp_header_generic))) {
	...
	if(message_len <= (payload_len-sizeof(struct gtp_header_generic))) {
	  NDPI_LOG(NDPI_OLD_RESULT_APP_GTP, ndpi_struct, NDPI_LOG_DEBUG, "Found gtp.\n");
	  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_GTP, NDPI_REAL_PROTOCOL);
	  return;
	}
      }
    }
  ...
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_GTP);
  return;
}
...
void ndpi_search_gtp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_GTP, ndpi_struct, NDPI_LOG_DEBUG, "gtp detection...\n");

  /* skip marked packets */
  if(packet->detected_protocol_stack[0] != NDPI_OLD_RESULT_APP_GTP)
