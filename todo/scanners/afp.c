...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_AFP, NDPI_REAL_PROTOCOL);
}
...
//  struct ndpi_id_struct *dst = flow->dst;
	...
	if (packet->payload_packet_len >= 22 && get_u_int16_t(packet->payload, 0) == htons(0x0004) &&
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_AFP, ndpi_struct, NDPI_LOG_DEBUG, "AFP: DSI OpenSession detected.\n");
	...
	if (packet->payload_packet_len >= 18 && get_u_int16_t(packet->payload, 0) == htons(0x0003) &&
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_AFP, ndpi_struct, NDPI_LOG_DEBUG, "AFP: DSI GetStatus detected.\n");
		ndpi_int_afp_add_connection(ndpi_struct, flow);
		return;
	}


	NDPI_LOG(NDPI_OLD_RESULT_APP_AFP, ndpi_struct, NDPI_LOG_DEBUG, "AFP excluded.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_AFP);
}
