...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_DOFUS, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len == 13 && get_u_int16_t(packet->payload, 1) == ntohs(0x0508)
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_DOFUS, ndpi_struct, NDPI_LOG_DEBUG, "found dofus.\n");
	...
	if (flow->l4.tcp.dofus_stage == 0 && packet->payload_packet_len == 3 && memcmp(packet->payload, "HG", 2) == 0
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_DOFUS, ndpi_struct, NDPI_LOG_DEBUG, "maybe dofus.\n");
		return;
	}
	...
	if (flow->l4.tcp.dofus_stage == 1 && packet->payload_packet_len == 5
		&& packet->payload[0] == 'A' && packet->payload[4] == 0x00 && (packet->payload[1] == 'T'
																	   || packet->payload[1] == 'k')) {
		NDPI_LOG(NDPI_OLD_RESULT_APP_DOFUS, ndpi_struct, NDPI_LOG_DEBUG, "found dofus asym.\n");
		ndpi_dofus_add_connection(ndpi_struct, flow);
		return;
	}
	/* end Dofus 1.x.x */
  ...
  exclude:
	NDPI_LOG(NDPI_OLD_RESULT_APP_DOFUS, ndpi_struct, NDPI_LOG_DEBUG, "exclude dofus.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_DOFUS);
}
