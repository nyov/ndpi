...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_SYSLOG, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "search syslog\n");

	if (packet->payload_packet_len > 20 && packet->payload_packet_len <= 1024 && packet->payload[0] == '<') {
		NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "checked len>20 and <1024 and first symbol=<.\n");
		i = 1;

		for (;;) {
			if (packet->payload[i] < '0' || packet->payload[i] > '9' || i++ > 3) {
				NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG,
		...
		if (packet->payload[i++] != '>') {
			NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "there is no > following the number.\n");
			NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_SYSLOG);
			return;
		} else {
			NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "a > following the number.\n");
		}

		if (packet->payload[i] == 0x20) {
			NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "a blank following the >: increment i.\n");
			i++;
		} else {
			NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "no blank following the >: do nothing.\n");
		}
		...
		if (i + sizeof("last message") - 1 <= packet->payload_packet_len &&
			...
			NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "found syslog by 'last message' string.\n");
			...
			NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "found syslog by 'snort: ' string.\n");
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_SYSLOG, ndpi_struct, NDPI_LOG_DEBUG, "no syslog detected.\n");
