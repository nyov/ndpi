...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_FIESTA, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_FIESTA, ndpi_struct, NDPI_LOG_DEBUG, "search fiesta.\n");
	...
	if (flow->l4.tcp.fiesta_stage == 0 && packet->payload_packet_len == 5
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_FIESTA, ndpi_struct, NDPI_LOG_DEBUG, "maybe fiesta symmetric, first packet.\n");
	...
	if (flow->l4.tcp.fiesta_stage == (2 - packet->packet_direction)
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_FIESTA, ndpi_struct, NDPI_LOG_DEBUG, "Maybe fiesta.\n");
		goto maybe_fiesta;
	}
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_FIESTA, ndpi_struct, NDPI_LOG_DEBUG, "exclude fiesta.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_FIESTA);
	return;

  maybe_fiesta:
	NDPI_LOG(NDPI_OLD_RESULT_APP_FIESTA, ndpi_struct, NDPI_LOG_DEBUG, "Stage is set to %d.\n", flow->l4.tcp.fiesta_stage);
	return;

  add_fiesta:
	NDPI_LOG(NDPI_OLD_RESULT_APP_FIESTA, ndpi_struct, NDPI_LOG_DEBUG, "detected fiesta.\n");
