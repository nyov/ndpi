...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_QUAKE, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if ((packet->payload_packet_len == 14
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_QUAKE, ndpi_struct, NDPI_LOG_DEBUG, "Quake IV detected.\n");
	...
	if (packet->payload_packet_len == 15 && get_u_int32_t(packet->payload, 0) == 0xffffffff
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_QUAKE, ndpi_struct, NDPI_LOG_DEBUG, "Quake III Arena/Quake Live detected.\n");
		ndpi_int_quake_add_connection(ndpi_struct, flow);
		return;
	}
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_QUAKE, ndpi_struct, NDPI_LOG_DEBUG, "Quake excluded.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_QUAKE);
}
