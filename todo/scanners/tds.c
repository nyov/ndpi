...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_TDS, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
		...
		if (flow->l4.tcp.tds_stage == 0) {
			...
			case 0x12:
				if (packet->payload[0] == 0x12) {
					NDPI_LOG(NDPI_OLD_RESULT_APP_TDS, ndpi_struct, NDPI_LOG_DEBUG, "TDS detected\n");
  ...
  exclude_tds:
	...
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_TDS);
}
