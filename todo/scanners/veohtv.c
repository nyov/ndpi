...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_VEOHTV, protocol_type);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_VEOHTV)
		return;
	...
	if (flow->l4.tcp.veoh_tv_stage == 1 || flow->l4.tcp.veoh_tv_stage == 2) {
		if (packet->packet_direction != flow->setup_packet_direction &&
			...
			if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_CONTENT_FLASH &&
				...
				NDPI_LOG(NDPI_OLD_RESULT_APP_VEOHTV, ndpi_struct, NDPI_LOG_DEBUG, "VeohTV detected.\n");
		...
		if (packet->payload_packet_len == 28 &&
			...
			NDPI_LOG(NDPI_OLD_RESULT_APP_VEOHTV, ndpi_struct, NDPI_LOG_DEBUG, "UDP VeohTV found.\n");
	...
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_VEOHTV);
}
#endif
