...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_STUN, NDPI_REAL_PROTOCOL);
}
													  ...
													  const u_int8_t * payload, const u_int16_t payload_length)
{
	...
	if (payload_length >= 20 && ntohs(get_u_int16_t(payload, 2)) + 20 == payload_length &&
		...
		u_int8_t old = 1;
		u_int8_t padding = 0;
		NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "len and type match.\n");

		if (payload_length == 20) {
			NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "found stun.\n");
			return NDPI_IS_STUN;
		}
		...
		while (a < payload_length) {
			...
			if (old && payload_length >= a + 4
				...
				NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "attribute match.\n");
				...
				NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "New STUN - attribute match.\n");
...
void ndpi_search_stun(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "search stun.\n");
	...
	if (packet->tcp) {
		...
		if (packet->payload_packet_len >= 2 + 20 &&
			...
			if (ndpi_int_check_stun(ndpi_struct, packet->payload + 2, packet->payload_packet_len - 2) ==
				NDPI_IS_STUN) {
				NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "found TCP stun.\n");
	...
	if (ndpi_int_check_stun(ndpi_struct, packet->payload, packet->payload_packet_len) == NDPI_IS_STUN) {
		NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "found UDP stun.\n");
		ndpi_int_stun_add_connection(ndpi_struct, flow);
		return;
	}

	NDPI_LOG(NDPI_OLD_RESULT_APP_STUN, ndpi_struct, NDPI_LOG_DEBUG, "exclude stun.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_STUN);
}
