					    ...
					    struct ndpi_flow_struct *flow, ndpi_protocol_type_t protocol_type)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_THUNDER, protocol_type);
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len > 8 && packet->payload[0] >= 0x30
		...
		if (flow->thunder_stage == 3) {
			NDPI_LOG(NDPI_OLD_RESULT_APP_THUNDER, ndpi_struct, NDPI_LOG_DEBUG, "THUNDER udp detected\n");
			ndpi_int_thunder_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
			return;
		}

		flow->thunder_stage++;
		NDPI_LOG(NDPI_OLD_RESULT_APP_THUNDER, ndpi_struct, NDPI_LOG_DEBUG,
	...
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_THUNDER);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len > 8 && packet->payload[0] >= 0x30
		&& packet->payload[0] < 0x40 && packet->payload[1] == 0 && packet->payload[2] == 0 && packet->payload[3] == 0) {
		if (flow->thunder_stage == 3) {
			NDPI_LOG(NDPI_OLD_RESULT_APP_THUNDER, ndpi_struct, NDPI_LOG_DEBUG, "THUNDER tcp detected\n");
			ndpi_int_thunder_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
			return;
		}
												  ...
												  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_THUNDER) {
	...
	if (packet->payload_packet_len > 5
		&& memcmp(packet->payload, "GET /", 5) == 0 && NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_OLD_RESULT_APP_THUNDER)) {
		NDPI_LOG(NDPI_OLD_RESULT_APP_THUNDER, ndpi_struct, NDPI_LOG_DEBUG, "HTTP packet detected.\n");
		ndpi_parse_packet_line_info(ndpi_struct, flow);
