...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_FLORENSIA, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_FLORENSIA, ndpi_struct, NDPI_LOG_DEBUG, "search florensia.\n");
	...
	if (packet->tcp != NULL) {
		if (packet->payload_packet_len == 5 && get_l16(packet->payload, 0) == packet->payload_packet_len
			...
			if (flow->florensia_stage == 1) {
				NDPI_LOG(NDPI_OLD_RESULT_APP_FLORENSIA, ndpi_struct, NDPI_LOG_DEBUG, "found florensia.\n");
				ndpi_florensia_add_connection(ndpi_struct, flow);
				return;
			}
			NDPI_LOG(NDPI_OLD_RESULT_APP_FLORENSIA, ndpi_struct, NDPI_LOG_DEBUG, "maybe florensia -> stage is set to 1.\n");
		...
		if (flow->florensia_stage == 1) {
			if (packet->payload_packet_len == 8 && get_l16(packet->payload, 0) == packet->payload_packet_len
				...
				NDPI_LOG(NDPI_OLD_RESULT_APP_FLORENSIA, ndpi_struct, NDPI_LOG_DEBUG, "found florensia asymmetrically.\n");
				ndpi_florensia_add_connection(ndpi_struct, flow);
				return;
			}
			...
			if (flow->packet_counter < 10 && get_l16(packet->payload, 0) == packet->payload_packet_len) {
				NDPI_LOG(NDPI_OLD_RESULT_APP_FLORENSIA, ndpi_struct, NDPI_LOG_DEBUG, "maybe florensia.\n");
				return;
			}
		}
	}
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_FLORENSIA, ndpi_struct, NDPI_LOG_DEBUG, "exclude florensia.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_FLORENSIA);
}
