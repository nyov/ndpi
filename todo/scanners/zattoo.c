					   ...
					   ndpi_protocol_type_t protocol_type)
{
  ...
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_ZATTOO, protocol_type);
...
u_int8_t ndpi_int_zattoo_user_agent_set(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  if (flow->packet.user_agent_line.ptr != NULL && flow->packet.user_agent_line.len == 111) {
    if (memcmp(flow->packet.user_agent_line.ptr +
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG, "found zattoo useragent\n");
      return 1;
    }
  }
  return 0;
...
void ndpi_search_zattoo(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_ZATTOO) {
  ...
  if (packet->tcp != NULL) {
    if (packet->payload_packet_len > 50 && memcmp(packet->payload, "GET /frontdoor/fd?brand=Zattoo&v=", 33) == 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct,
	...
	return;
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG, "add connection over tcp with 0x0304.\n");
      ndpi_int_zattoo_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
      return;
	...
	return;
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG,
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG, "detected zattoo.\n");
      ndpi_int_zattoo_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
      return;
    } else if (flow->zattoo_stage == 6 - packet->packet_direction && packet->payload_packet_len == 1412) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG, "found zattoo.\n");
      ndpi_int_zattoo_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
      return;
    }
    NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG,
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG, "add connection over udp.\n");
	ndpi_int_zattoo_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	return;
      }
      NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG, "need next packet udp.\n");
      return;
    }
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_ZATTOO, ndpi_struct, NDPI_LOG_DEBUG, "exclude zattoo.\n");
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_ZATTOO);
}
#endif
