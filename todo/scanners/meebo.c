...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_MEEBO, NDPI_CORRELATED_PROTOCOL);
}
		       ...
		       *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "search meebo.\n");

  /* catch audio/video flows which are flash (rtmp) */
  if (
#ifdef NDPI_OLD_RESULT_CONTENT_FLASH
      packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_CONTENT_FLASH
#else
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "found meebo/tokbox flash flow.\n");
    ...
    if (flow->packet_counter < 16 && flow->packet_direction_counter[flow->setup_packet_direction] < 6) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "need next packet.\n");
      return;
    }

    NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "exclude meebo.\n");
    NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_MEEBO);
    return;
  }
  ...
  if ((
	...
	&& memcmp(&packet->host_line.ptr[packet->host_line.len - 9], "meebo.com", 9) == 0) {
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "Found Meebo host\n");
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "Found tokbox host\n");
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "Found meebo IP\n");
      host_or_referer_match = 1;
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "Found meebo referer\n");
      host_or_referer_match = 1;
    } else if (packet->referer_line.ptr != NULL &&
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "Found tokbox referer\n");
      host_or_referer_match = 1;
    } else if (packet->referer_line.ptr != NULL &&
      ...
      NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "Found meebo IP referer\n");
      host_or_referer_match = 1;
    }

    if (host_or_referer_match) {
      if (host_or_referer_match == 1) {
	NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG,
  ...
  if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_MEEBO) {
    NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG,
	     "in case that ssl meebo has been detected return.\n");
    return;
  }

  if (flow->packet_counter < 5 && packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_UNKNOWN
      && NDPI_COMPARE_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_BASE_SSL) == 0) {
    NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "ssl not yet excluded. need next packet.\n");
    return;
  }
#ifdef NDPI_OLD_RESULT_CONTENT_FLASH
  if (flow->packet_counter < 5 && packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_UNKNOWN &&
      !NDPI_FLOW_PROTOCOL_EXCLUDED(ndpi_struct, flow, NDPI_OLD_RESULT_CONTENT_FLASH)) {
    NDPI_LOG(NDPI_OLD_RESULT_APP_MEEBO, ndpi_struct, NDPI_LOG_DEBUG, "flash not yet excluded. need next packet.\n");
    return;
  }
#endif
