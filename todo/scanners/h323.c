...
void ndpi_search_h323(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_OLD_RESULT_APP_H323, ndpi_struct, NDPI_LOG_DEBUG, "search H323.\n");

  if(packet->tcp != NULL) {
    NDPI_LOG(NDPI_OLD_RESULT_APP_H323, ndpi_struct, NDPI_LOG_DEBUG, "calculated dport over tcp.\n");

    /* H323  */
    if(packet->payload[0] == 0x03 && packet->payload[1] == 0x00 && packet->payload[2] == 0x00)
      {
	NDPI_LOG(NDPI_OLD_RESULT_APP_H323, ndpi_struct, NDPI_LOG_DEBUG, "found H323 broadcast.\n");
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_H323, NDPI_REAL_PROTOCOL);
	return;
      }
  }
  ...
  if(packet->udp != NULL) {
    ...
    NDPI_LOG(NDPI_OLD_RESULT_APP_H323, ndpi_struct, NDPI_LOG_DEBUG, "calculated dport over udp.\n");
    ...
    if(sport == 1719 || dport == 1719)
	  ...
	  {
	    NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_H323);
	    return;
	  }
      }
  }
