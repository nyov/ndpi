...
void ndpi_search_mail_imap(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->payload_packet_len >= 4 && ntohs(get_u_int16_t(packet->payload, packet->payload_packet_len - 2)) == 0x0d0a) {
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "mail imap identified\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_IMAP;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_IMAP] = 1;
	return;
      }
    }
  }
 ...
 imap_excluded:
  ...
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "exclude IMAP.\n");
  flow->ndpi_excluded_app[NDPI_RESULT_APP_IMAP] = 1;
}

void ndpi_register_proto_imap (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {143, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_IMAP, "IMAP", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_mail_imap);
}
