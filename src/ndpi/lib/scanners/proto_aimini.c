...
void ndpi_search_aimini(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "search aimini.\n");
	...
	if (packet->udp != NULL) {
		if (flow->l4.udp.aimini_stage == 0) {
			if (packet->payload_packet_len == 64 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x010b) {
				...
				NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 1.\n");
				return;
			}
		...
		if (flow->l4.udp.aimini_stage == 1 && packet->payload_packet_len > 100 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x0115) {
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 2.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 2 &&
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 3.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 3
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found aimini (64, 0x010b), (>300, 0x0115), "
					"(16, 0x010c || 64, 0x010b), (16, 0x010c || 64, 0x010b || >100, 0x0115).\n");
			flow->ndpi_result_app = NDPI_RESULT_APP_AIMINI;
			flow->ndpi_excluded_app[NDPI_RESULT_APP_AIMINI] = 1;
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 4 && packet->payload_packet_len == 136
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 5.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 5 && (packet->payload_packet_len == 136
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 6.\n");
			return;
		}
		
		if (flow->l4.udp.aimini_stage == 6 && ((packet->payload_packet_len == 136 && ((ntohs(get_u_int16_t(packet->payload, 0)) == 0x0165)
				|| ntohs(get_u_int16_t(packet->payload, 0)) == 0x01c9)) || (packet->payload_packet_len == 32
				&& ntohs(get_u_int16_t(packet->payload, 0)) == 0x01ca))) {
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found aimini (136, 0x01c9), (136, 0x01c9)," "(136, 0x01c9),(136, 0x01c9 || 32, 0x01ca).\n");
			flow->ndpi_result_app = NDPI_RESULT_APP_AIMINI;
			flow->ndpi_excluded_app[NDPI_RESULT_APP_AIMINI] = 1;
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 7 && packet->payload_packet_len == 88 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x0101) {
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 8.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 8 && (packet->payload_packet_len == 88 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x0101)) {
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 9.\n");
			return;
		}
		
		if (flow->l4.udp.aimini_stage == 9 && (packet->payload_packet_len == 88 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x0101)) {
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found aimini (88, 0x0101), (88, 0x0101)," "(88, 0x0101),(88, 0x0101).\n");
			flow->ndpi_result_app = NDPI_RESULT_APP_AIMINI;
			flow->ndpi_excluded_app[NDPI_RESULT_APP_AIMINI] = 1;
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 10 && packet->payload_packet_len == 104 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x0102) {
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 11.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 11
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 12.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 12 && ((packet->payload_packet_len == 104 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x0102)
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found aimini (104, 0x0102), (104, 0x0102), " "(104, 0x0102), (104, 0x0102).\n");
			flow->ndpi_result_app = NDPI_RESULT_APP_AIMINI;
			flow->ndpi_excluded_app[NDPI_RESULT_APP_AIMINI] = 1;
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 13 && packet->payload_packet_len == 32 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x01ca) {
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 14.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 14 && ((packet->payload_packet_len == 32 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x01ca)
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 15.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 16 && packet->payload_packet_len == 16 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x010c) {
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 17.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 17 && (packet->payload_packet_len == 16 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x010c)) {
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "stage = 18.\n");
			return;
		}
		...
		if ((packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /player/") &&
			...
			if (packet->host_line.ptr != NULL && packet->host_line.len > 11
				...
				NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "AIMINI HTTP traffic detected.\n");
				flow->ndpi_result_app = NDPI_RESULT_APP_AIMINI;
				flow->ndpi_excluded_app[NDPI_RESULT_APP_AIMINI] = 1;
				return;
			}
		...
		if (packet->payload_packet_len > 100) {
			if (memcmp(packet->payload, "GET /", NDPI_STATICSTRING_LEN("GET /")) == 0) {
				if (memcmp(&packet->payload[NDPI_STATICSTRING_LEN("GET /")], "play/",
					...
					memcmp(&packet->payload[NDPI_STATICSTRING_LEN("GET /")], "download/", NDPI_STATICSTRING_LEN("download/")) == 0) {
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "exclude aimini.\n");
	flow->ndpi_excluded_app[NDPI_RESULT_APP_AIMINI] = 1;
}

void ndpi_register_proto_aimini (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_AIMINI, "Aimini", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_OR_UDP_WITH_PAYLOAD, tcp_ports, udp_ports, ndpi_search_aimini);
}
