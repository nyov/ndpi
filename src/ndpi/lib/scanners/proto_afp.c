...
void ndpi_search_afp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
  struct ndpi_packet_struct *packet = &flow->packet;
	...
	if (packet->payload_packet_len >= 22 && get_u_int16_t(packet->payload, 0) == htons(0x0004) &&
		...
		NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "AFP: DSI OpenSession detected.\n");
		flow->ndpi_result_app = NDPI_RESULT_APP_AFP;
		flow->ndpi_excluded_app[NDPI_RESULT_APP_AFP] = 1;
		return;
	}
	...
	if (packet->payload_packet_len >= 18 && get_u_int16_t(packet->payload, 0) == htons(0x0003) &&
		...
		NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "AFP: DSI GetStatus detected.\n");
		flow->ndpi_result_app = NDPI_RESULT_APP_AFP;
		flow->ndpi_excluded_app[NDPI_RESULT_APP_AFP] = 1;
		return;
	}

	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "AFP excluded.\n");
	flow->ndpi_excluded_app[NDPI_RESULT_APP_AFP] = 1;
}

void ndpi_register_proto_afp (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_AFP, "AFP", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_afp);
}
