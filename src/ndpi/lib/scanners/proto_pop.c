...
void ndpi_search_mail_pop(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
	...
	if (packet->payload_packet_len > 2 && ntohs(get_u_int16_t(packet->payload, packet->payload_packet_len - 2)) == 0x0d0a) {
		...
		if ((bit_count + flow->l4.tcp.mail_pop_stage) >= 3) {
			if (flow->l4.tcp.mail_pop_stage > 0) {
				NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "mail_pop identified\n");
				flow->ndpi_result_app = NDPI_RESULT_APP_POP;
				flow->ndpi_excluded_app[NDPI_RESULT_APP_POP] = 1;
				return;
			} else {
				return;
			}
  ...
  maybe_split_pop:
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "exclude mail_pop\n");
	flow->ndpi_excluded_app[NDPI_RESULT_APP_POP] = 1;
}

void ndpi_register_proto_pop (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {110, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_POP, "POP3", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_mail_pop);
}
