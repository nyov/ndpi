...
void ndpi_search_ftp_control(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
  ...
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "FTP_CONTROL detection...\n");
  
  /* Exclude SMTP, which uses similar commands. */
  if (packet->tcp->dest == htons(25) || packet->tcp->source == htons(25)) {
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Exclude FTP_CONTROL.\n");
    flow->ndpi_excluded_app[NDPI_RESULT_APP_FTP_CONTROL] = 1;
    return;
  }
  ...
  if (flow->ftp_control_stage == 0) {
     NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "FTP_CONTROL stage 0: \n");
     
     if ((payload_len > 0) && ndpi_ftp_control_check_request(packet->payload)) {
       NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Possible FTP_CONTROL request detected, we will look further for the response...\n");
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "FTP_CONTROL stage %u: \n", flow->ftp_control_stage);
    ...
    if ((payload_len > 0) && ndpi_ftp_control_check_response(packet->payload)) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Found FTP_CONTROL.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_FTP_CONTROL;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_FTP_CONTROL] = 1;
    } else {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "The reply did not seem to belong to FTP_CONTROL, resetting the stage to 0...\n");
...
void ndpi_register_proto_ftp_control (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {21, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_FTP_CONTROL, "FTP_Control", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_ftp_control);
}
