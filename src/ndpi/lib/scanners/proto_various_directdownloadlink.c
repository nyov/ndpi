/*
 * proto_various_directdownloadlink.c
 *
...
void ndpi_search_direct_download_link(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->payload_packet_len < 100) {
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: Packet too small.\n");
    goto end_ddl_nothing_found;
  }

  if (memcmp(packet->payload, "POST ", 5) == 0) {
    filename_start = 5;		// POST
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: POST FOUND\n");
  } else if (memcmp(packet->payload, "GET ", 4) == 0) {
    filename_start = 4;		// GET
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: GET FOUND\n");
  } else {
    goto end_ddl_nothing_found;
  }
  // parse packet
  ...
  if (packet->host_line.ptr == NULL) {
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: NO HOST FOUND\n");
    goto end_ddl_nothing_found;
  }

  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: Host: found\n");
  ...
  if (host_line_len_without_port >= i && packet->host_line.ptr[host_line_len_without_port - i] >= '0'
    ...
    while (host_line_len_without_port >= i && packet->host_line.ptr[host_line_len_without_port - i] >= '0'
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: number found\n");
      i++;
    }
    if (host_line_len_without_port >= i && packet->host_line.ptr[host_line_len_without_port - i] == ':') {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: ':' found\n");
 ...
 end_ddl_nothing_found:
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Nothing Found\n%.*s\n", packet->payload_packet_len, packet->payload);
  
  /* Break after 10 packets. */
  if (flow->packet_counter > 10) {
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Exclude Direct Download.\n");
    flow->ndpi_excluded_app[NDPI_RESULT_APP_DIRECT_DOWNLOAD] = 1;
  }  
  
  return;

 end_ddl_found:
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "DDL: DIRECT DOWNLOAD LINK FOUND\n");
  flow->ndpi_result_app = NDPI_RESULT_APP_DIRECT_DOWNLOAD;
  flow->ndpi_excluded_app[NDPI_RESULT_APP_DIRECT_DOWNLOAD] = 1;
  return;
}

void ndpi_register_proto_various_directdownloadlink (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_DIRECT_DOWNLOAD, "Direct_Download", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD, tcp_ports, udp_ports, ndpi_search_direct_download_link);
}
