...
void ndpi_search_netbios(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->udp != NULL) {
    ...
    if ((dport == 137 || 0) && packet->payload_packet_len >= 50) {
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found netbios port 137 and payload_packet_len 50\n");
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found netbios with questions = 1 and answers = 0, authority = 0  \n");
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found netbios with questions = 1 and answers, authority, additional = 0  \n");
	flow->ndpi_result_app = NDPI_RESULT_APP_NETBIOS;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_NETBIOS] = 1;
	return;
      }
	...
	if (get_u_int16_t(packet->payload, 4) == htons(1) &&
	  ...
	  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "possible netbios name query request, one question\n");

	  if (packet->payload_packet_len == 12 + 1 + name_length + 1 + 2 + 2) {

	    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "possible netbios name query request, length matches\n");
		...
		NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found netbios name query request\n");
		flow->ndpi_result_app = NDPI_RESULT_APP_NETBIOS;
		flow->ndpi_excluded_app[NDPI_RESULT_APP_NETBIOS] = 1;
	      return;
	    }
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "possible netbios name query response\n");
	...
	if (get_u_int16_t(packet->payload, 4) == 0 &&
	  ...
	  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "possible netbios positive name query response, one answer\n");

	  if (packet->payload_packet_len >= 12 + 1 + name_length + 1 + 2 + 2) {

	    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "possible netbios name query response, length matches\n");
		...
		get_u_int16_t(packet->payload, 12 + name_length + 4) == htons(0x0001)) {
	      ...
	      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found netbios name query response\n");
	      flow->ndpi_result_app = NDPI_RESULT_APP_NETBIOS;
	      flow->ndpi_excluded_app[NDPI_RESULT_APP_NETBIOS] = 1;
	      return;
	    }
	  ...
	  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "possible netbios negative name query response, one answer\n");
	  ...
	  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "possible netbios redirect name query response, one answer\n");
    ...
    if ((dport == 138) && packet->payload_packet_len >= 14 && ntohs(get_u_int16_t(packet->payload, 10)) == packet->payload_packet_len - 14) {
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found netbios port 138 and payload length >= 112 \n");

      if (packet->payload[0] >= 0x11 && packet->payload[0] <= 0x16) {

	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found netbios with MSG-type 0x11,0x12,0x13,0x14,0x15 or 0x16\n");
