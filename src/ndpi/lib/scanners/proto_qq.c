...
static void ndpi_search_qq_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
  ...
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "search qq udp.\n");
  ...
  if (flow->qq_stage <= 3) {
	...
	|| (packet->payload_packet_len > 10
      ...
      if (flow->qq_stage == 3) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 030001 or 000e35 four times.\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
	...
	if (pat == p8000_patt_02[index] && packet->payload[packet->payload_packet_len - 1] == 0x03) {
	  ...
	  if (flow->qq_stage == 3) {
	    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 02 ... 03 four times.\n");
	    flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	    flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	    return;
	  }
	...
	if (pat == p8000_patt_02[index]) {
	  ...
	  if (flow->qq_stage == 3) {
	    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern four times.\n");
	    flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	    flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	    return;
	  }
	...
	&& packet->payload[packet->payload_packet_len - 1] == 0x03) {
      ...
      if (flow->qq_stage == 3) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 04 1159 ... 03 four times.\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
	...
	&& (packet->payload[packet->payload_packet_len - 1] == 0x00
      ...
      if (flow->qq_stage == 3) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 02/06 0100 ... 03/00 four times.\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
	...
	&& ntohs(get_u_int16_t(packet->payload, 1)) == 0x1131 && packet->payload[packet->payload_packet_len - 1] == 0x03) {
      ...
      if (flow->qq_stage == 3) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 02 1131 ... 03 four times.\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
	...
	get_u_int16_t(packet->payload, 4) == htons(0x0b0b)) {
      ...
      if (flow->qq_stage == 3) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 0203[packet_length_0b0b] three times.\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
	...
	if (flow->qq_stage == 3) {
	  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 02 02 <length> four times.\n");
	  flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	  flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	  return;
	}
  ...
  if (ndpi_is_valid_qq_packet(packet)) {
    ...
    if (flow->qq_stage == 3) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq over udp.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
      return;
    }
    
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq packet stage %d\n", flow->qq_stage);
    return;
  }
  ...
  if (ndpi_is_valid_qq_ft_packet(packet)) {
    ...
    if (flow->qq_stage == 3) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq ft over udp.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
      return;
    }
  ...
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "QQ excluded\n");
  flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
}
...
void ndpi_search_qq_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "search qq tcp.\n");
  ...
  if ((packet->payload_packet_len > 4 && ntohs(get_u_int16_t(packet->payload, 0)) == packet->payload_packet_len
    ...
    if (flow->qq_stage == 3) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
      return;
    }
  ...
  if (ndpi_is_valid_qq_ft_packet(packet)) {
    ...
    if (flow->qq_stage == 3) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq ft over tcp.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
      return;
    }
  ...
  if (packet->payload_packet_len > 100
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found GET or POST.\n");
  
    if (memcmp(packet->payload, "GET /qqfile/qq", 14) == 0) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp GET /qqfile/qq.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
      return;
    }
	...
	&& (packet->user_agent_line.len > 7 && memcmp(packet->user_agent_line.ptr, "QQClient", 8) == 0)) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp GET...QQClient\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
      return;
    }
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp GET...QQ: \n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
    ...
    if (packet->host_line.ptr != NULL) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "host line ptr\n");
      
      if (packet->host_line.len > 11 && memcmp(&packet->host_line.ptr[0], "www.qq.co.za", 12) == 0) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp Host: www.qq.co.za\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
  ...
  if (flow->qq_stage == 0 && packet->payload_packet_len == 82
	...
	break;
      ...
      if (i == 81) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq Mail.\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
  ...
  if (packet->payload_packet_len == 204 && flow->qq_stage == 0 && get_u_int32_t(packet->payload, 200) == htonl(0xfbffffff)) {
    for (i = 0; i < 200; i++) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "i = %u\n", i);
	...
	break;
      ...
      if (i == 199) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found qq chat or file transfer\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_QQ;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
	return;
      }
  ...
  if (flow->ndpi_excluded_base[NDPI_RESULT_BASE_HTTP] == 1) {
    flow->ndpi_excluded_app[NDPI_RESULT_APP_QQ] = 1;
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "QQ tcp excluded; len %u\n", packet->payload_packet_len);
  }
...
void ndpi_register_proto_qq (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_QQ, "QQ", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD, tcp_ports, udp_ports, ndpi_search_qq);
}
