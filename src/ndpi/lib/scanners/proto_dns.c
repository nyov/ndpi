...
#define NDPI_MAX_DNS_REQUESTS			16
  ...
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "search DNS.\n");
  ...
  if(((dport == 53) || (sport == 53) || (dport == 5355))
	...
	printf("==> %s\n", flow->host_server_name);
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found DNS.\n");
	
      if (dport == 5355) {
	flow->ndpi_result_app = NDPI_RESULT_APP_LLMNR;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_DNS] = 1;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_LLMNR] = 1;
      } else {
	flow->ndpi_result_app = NDPI_RESULT_APP_DNS;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_DNS] = 1;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_LLMNR] = 1;	  
      }
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "exclude DNS.\n");
      flow->ndpi_excluded_app[NDPI_RESULT_APP_DNS] = 1;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_LLMNR] = 1;
    }
  }
...
void ndpi_register_proto_dns (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {53, 5355, 0, 0, 0};
  int udp_ports[5] = {53, 5355, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_DNS, "DNS", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_OR_UDP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_dns);
  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_LLMNR, "LLMNR", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_OR_UDP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, NULL);
}
