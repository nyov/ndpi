...
void ndpi_search_tds(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
		...
		if (flow->l4.tcp.tds_stage == 0) {
			...
			case 0x12:
				if (packet->payload[0] == 0x12) {
					NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "TDS detected\n");
					flow->ndpi_result_app = NDPI_RESULT_APP_TDS;
					flow->ndpi_excluded_app[NDPI_RESULT_APP_TDS] = 1;
					return;
				} else {
					goto exclude_tds;
				}
...
void ndpi_register_proto_tds (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_TDS, "TDS", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_tds);
}
