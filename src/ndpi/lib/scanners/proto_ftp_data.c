...
void ndpi_search_ftp_data(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
	...
	if (flow->packet_counter > 20) {
		NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Exclude FTP_DATA.\n");
		flow->ndpi_excluded_app[NDPI_RESULT_APP_FTP_DATA] = 1;
		return;
	}
	
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "FTP_DATA detection...\n");
	
	/* It is similar to HTTP, so exclude, if HTTP was detected before. */
	...
	if (flow->ftp_data_stage == 0) {
		NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "FTP_DATA stage 0: \n");
		...
		if ((payload_len > 0) && (ndpi_match_file_header(ndpi_struct, flow) || ndpi_match_ftp_data_directory(ndpi_struct, flow) || ndpi_match_ftp_data_port(ndpi_struct, flow))) {
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Possible FTP_DATA request detected, we will look further for the response...\n");
		...
		NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "FTP_DATA stage %u: \n", flow->ftp_data_stage);
		...
		if (payload_len == 0) {
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Found FTP_DATA.\n");
			flow->ndpi_result_app = NDPI_RESULT_APP_FTP_DATA;
			flow->ndpi_excluded_app[NDPI_RESULT_APP_FTP_DATA] = 1;
		} else {
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "The reply did not seem to belong to FTP_DATA, resetting the stage to 0...\n");
...
void ndpi_register_proto_ftp_data (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {20, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_FTP_DATA, "FTP_Data", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_ftp_data);
}
