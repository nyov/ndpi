...
void ndpi_search_imesh_tcp_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->udp != NULL) {
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "UDP FOUND\n");
	...
	(packet->udp->dest == htons(1864) || packet->udp->source == htons(1864))) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "iMesh Login detected\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_IMESH;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_IMESH] = 1;
      return;
    }
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "iMesh detected\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_IMESH;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_IMESH] = 1;
	return;
      }
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "iMesh UDP packetlen: %d\n",
  ...
  if (packet->tcp != NULL) {
	...
	get_u_int16_t(packet->payload, 49) == htons(0x001a)) {
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found imesh.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_IMESH;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_IMESH] = 1;
      return;
    }
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "TCP FOUND :: Payload %u\n",
	     packet->payload_packet_len);
  ...
  //imesh_not_found_end:
  flow->ndpi_excluded_app[NDPI_RESULT_APP_IMESH] = 1;
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "iMesh excluded at stage %d\n",
	   packet->tcp != NULL ? flow->l4.tcp.imesh_stage : 0);

}

void ndpi_register_proto_imesh (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_IMESH, "iMESH", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_OR_UDP_WITH_PAYLOAD, tcp_ports, udp_ports, ndpi_search_imesh_tcp_udp);
}
