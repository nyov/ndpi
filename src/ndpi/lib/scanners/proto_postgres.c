...
void ndpi_search_postgres(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
	...
	if (flow->l4.tcp.postgres_stage == 0) {
		...
		if (flow->l4.tcp.postgres_stage == 2 - packet->packet_direction) {
			...
			if (packet->payload_packet_len == 1 && packet->payload[0] == 'S') {
				NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "PostgreSQL detected, SSL accepted.\n");
				flow->ndpi_result_app = NDPI_RESULT_APP_POSTGRES;
				flow->ndpi_excluded_app[NDPI_RESULT_APP_POSTGRES] = 1;
				return;
			}
			
			//SSL denied
			if (packet->payload_packet_len == 1 && packet->payload[0] == 'N') {
				NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "PostgreSQL detected, SSL denied.\n");
				flow->ndpi_result_app = NDPI_RESULT_APP_POSTGRES;
				flow->ndpi_excluded_app[NDPI_RESULT_APP_POSTGRES] = 1;
				return;
			}
		...
		if (flow->l4.tcp.postgres_stage == 4 - packet->packet_direction)
			if (packet->payload_packet_len > 8 &&
				...
				NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "PostgreSQL detected, no SSL.\n");
				flow->ndpi_result_app = NDPI_RESULT_APP_POSTGRES;
				flow->ndpi_excluded_app[NDPI_RESULT_APP_POSTGRES] = 1;
				return;
			}
		if (flow->l4.tcp.postgres_stage == 6
			...
			NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found postgres asymmetrically.\n");
			flow->ndpi_result_app = NDPI_RESULT_APP_POSTGRES;
			flow->ndpi_excluded_app[NDPI_RESULT_APP_POSTGRES] = 1;
			return;
		}
...
void ndpi_register_proto_postgres (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {5432, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_POSTGRES, "PostgreSQL", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, tcp_ports, udp_ports, ndpi_search_postgres);
}
