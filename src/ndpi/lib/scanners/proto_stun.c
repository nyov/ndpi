...
static u_int8_t ndpi_int_check_stun(struct ndpi_detection_module_struct *ndpi_struct, const u_int8_t * payload, const u_int16_t payload_length) {
  ...
  if((payload_length > 13)
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "Found stun.\n");
    return 1;
  }
  ...
  if (payload_length >= 20 && ntohs(get_u_int16_t(payload, 2)) + 20 == payload_length &&
    ...
    u_int8_t old = 1;
    u_int8_t padding = 0;
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "len and type match.\n");
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "attribute match.\n");
	...
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "New STUN - attribute match.\n");
...
void ndpi_search_stun(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
  ...
  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "search stun.\n");
  ...
  if (packet->tcp) {
    ...
    if (packet->payload_packet_len >= 2 + 20 && ntohs(get_u_int16_t(packet->payload, 0)) + 2 == packet->payload_packet_len) {
      ...
      if (ndpi_int_check_stun(ndpi_struct, packet->payload + 2, packet->payload_packet_len - 2) == 1) {
	NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found TCP stun.\n");
	flow->ndpi_result_app = NDPI_RESULT_APP_STUN;
	flow->ndpi_excluded_app[NDPI_RESULT_APP_STUN] = 1;
	return;
      }
    }
  }
  
  if (ndpi_int_check_stun(ndpi_struct, packet->payload, packet->payload_packet_len) == 1) {
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "found UDP stun.\n");
    flow->ndpi_result_app = NDPI_RESULT_APP_STUN;
    flow->ndpi_excluded_app[NDPI_RESULT_APP_STUN] = 1;
    return;
  }

  NDPI_LOG(0, ndpi_struct, NDPI_LOG_DEBUG, "exclude stun.\n");
  flow->ndpi_excluded_app[NDPI_RESULT_APP_STUN] = 1;
}

void ndpi_register_proto_stun (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_STUN, "STUN", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_OR_UDP_WITH_PAYLOAD, tcp_ports, udp_ports, ndpi_search_stun);
}
