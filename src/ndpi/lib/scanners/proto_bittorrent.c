						    ...
						    *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->payload_packet_len > 23 && memcmp(packet->payload, "GET /webseed?info_hash=", 23) == 0) {
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_TRACE, "BT: plain webseed BitTorrent protocol detected\n");
    flow->ndpi_result_app = NDPI_RESULT_APP_BITTORRENT;
    flow->ndpi_excluded_app[NDPI_RESULT_APP_BITTORRENT] = 1;
  ...
  if (packet->payload_packet_len > 60
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_TRACE, "BT: plain Bitcomet persistent seed protocol detected\n");
    flow->ndpi_result_app = NDPI_RESULT_APP_BITTORRENT;
    flow->ndpi_excluded_app[NDPI_RESULT_APP_BITTORRENT] = 1;
  ...
  if (packet->payload_packet_len > 90 && (memcmp(packet->payload, "GET ", 4) == 0
	...
	&& ((packet->user_agent_line.len > 8 && memcmp(packet->user_agent_line.ptr, "Azureus ", 8) == 0)
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_TRACE, "Azureus /Bittorrent user agent line detected\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_BITTORRENT;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_BITTORRENT] = 1;
	...
	&& (packet->parsed_lines > 8 && packet->line[8].ptr != 0
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_TRACE, "Bittorrent Shareaza detected.\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_BITTORRENT;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_BITTORRENT] = 1;
	...
	&& packet->line[8].len > 22 && memcmp(packet->line[8].ptr, "Cache-Control: no-cache", 23) == 0) {
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_TRACE, "Bitcomet LTS detected\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_BITTORRENT;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_BITTORRENT] = 1;
	...
	&& packet->line[6].len > 21 && memcmp(packet->line[6].ptr, "Connection: Keep-Alive", 22) == 0) {
      ...
      NDPI_LOG(0, ndpi_struct, NDPI_LOG_TRACE, "FlashGet detected\n");
      flow->ndpi_result_app = NDPI_RESULT_APP_BITTORRENT;
      flow->ndpi_excluded_app[NDPI_RESULT_APP_BITTORRENT] = 1;
    ...
    NDPI_LOG(0, ndpi_struct, NDPI_LOG_TRACE, " BT stat: tracker info hash parsed\n");
    flow->ndpi_result_app = NDPI_RESULT_APP_BITTORRENT;
    flow->ndpi_excluded_app[NDPI_RESULT_APP_BITTORRENT] = 1;
...
void ndpi_search_bittorrent(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if(packet->iph
    ...
    if ((packet->tcp != NULL) && (packet->tcp_retransmission == 0 || packet->num_retried_bytes)) {
      ndpi_int_search_bittorrent_tcp(ndpi_struct, flow);
    } else if (packet->udp != NULL) {
      if((ntohs(packet->udp->source) < 1024) || (ntohs(packet->udp->dest) < 1024) /* High ports only */)
	return;
...
void ndpi_register_proto_bittorrent (struct ndpi_detection_module_struct *ndpi_mod) {

  int tcp_ports[5] = {0, 0, 0, 0, 0};
  int udp_ports[5] = {0, 0, 0, 0, 0};

  ndpi_initialize_scanner_app (ndpi_mod, NDPI_RESULT_APP_BITTORRENT, "BitTorrent", NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_OR_UDP, tcp_ports, udp_ports, ndpi_search_bittorrent);
}
