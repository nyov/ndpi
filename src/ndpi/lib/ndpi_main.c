						  ...
						  u_int32_t flags)
{
  ...
  /* tcp / udp detection */
  if(key_return->protocol == 6  && l4_len >= sizeof(struct ndpi_tcphdr)) {
  ...
  } else if(key_return->protocol == 17  && l4_len >= sizeof(struct ndpi_udphdr)) {
...
u_int8_t ndpi_detection_build_key(const u_int8_t * l3, u_int16_t l3_len, const u_int8_t * l4, u_int16_t l4_len, u_int8_t l4_protocol,
  ...
  return ndpi_detection_build_key_internal(NULL, l3, l3_len, l4, l4_len, l4_protocol, key_return, dir_return, flags);
}
