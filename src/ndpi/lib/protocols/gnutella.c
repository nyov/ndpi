...
#ifdef NDPI_OLD_RESULT_APP_GNUTELLA
					     ...
					     ndpi_protocol_type_t protocol_type)
{
  ...
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_GNUTELLA, protocol_type);
  ...
  if (src != NULL) {
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct,
...
void ndpi_search_gnutella(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  u_int16_t c;
  if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_GNUTELLA) {
  ...
  if (packet->tcp != NULL) {
    ...
    if (packet->payload_packet_len > 10 && memcmp(packet->payload, "GNUTELLA/", 9) == 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_TRACE, "GNUTELLA DETECTED\n");
	...
	if ((packet->line[c].len > 19 && memcmp(packet->line[c].ptr, "User-Agent: Gnutella", 20) == 0)
	  ...
	  NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_DEBUG, "DETECTED GNUTELLA GET.\n");
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_TRACE,
    ...
    if (packet->payload_packet_len > 30 && memcmp(packet->payload, "HEAD /gnutella/push-proxy?", 26) == 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_TRACE, "detected HEAD /gnutella/push-proxy?\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
      return;
    }
	...
	while (c < end) {
	  if ((memcmp(&packet->payload[14], "tigertree/", 10) == 0)
	    ...
	    NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA,
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_TRACE, "MORPHEUS GIV DETECTED\n");
	...
	get_u_int16_t(packet->payload, 12) == htons(0x0034)) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_TRACE, "detected gnutella len == 46.\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
	...
	memcmp(packet->payload, "\x80\x2f\x01\x03\x01\x00\x06\x00\x00\x00\x20\x00\x00\x34\x00\x00\xff\x4d\x6c",
	       19) == 0) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_TRACE, "detected gnutella len == 49.\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_TRACE, "detected len == 82.\n");
	ndpi_int_gnutella_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	return;
      }
    }
	...
	(packet->tick_timestamp - src->gnutella_ts) < ndpi_struct->gnutella_timeout) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_DEBUG, "port based detection\n\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
    }
    /* observations:
	...
	&& packet->payload[20] == 0x00 && packet->payload[21] == 0x00 && packet->payload[22] == 0x00) {
      NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct, NDPI_LOG_DEBUG,
  ...
  if (packet->tcp != NULL && ntohs(packet->tcp->source) >= 1024 && ntohs(packet->tcp->dest) >= 1024) {
	...
	return;
	  ...
	  && NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_OLD_RESULT_APP_GNUTELLA)) {
	NDPI_LOG(NDPI_OLD_RESULT_APP_GNUTELLA, ndpi_struct,
  ...
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_GNUTELLA);
}
#endif
