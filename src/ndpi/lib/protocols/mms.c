...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_CONTENT_MMS, NDPI_REAL_PROTOCOL);
}
...
void ndpi_search_mms_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->payload_packet_len >= 20) {
	...
	&& packet->payload[13] == 0x4d && packet->payload[14] == 0x53 && packet->payload[15] == 0x20) {
      NDPI_LOG(NDPI_OLD_RESULT_CONTENT_MMS, ndpi_struct, NDPI_LOG_DEBUG, "MMS: MSMMS Request found \n");
	...
	&& packet->payload[14] == 0x53 && packet->payload[15] == 0x20) {
      NDPI_LOG(NDPI_OLD_RESULT_CONTENT_MMS, ndpi_struct, NDPI_LOG_DEBUG, "MMS: MSMMS Response found \n");
  ...
  if (NDPI_COMPARE_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_BASE_HTTP) != 0) {
    ...
    NDPI_LOG(NDPI_OLD_RESULT_CONTENT_MMS, ndpi_struct, NDPI_LOG_DEBUG, "MMS: exclude\n");
    NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_CONTENT_MMS);

#ifdef NDPI_OLD_RESULT_BASE_HTTP
  } else {
    NDPI_LOG(NDPI_OLD_RESULT_CONTENT_MMS, ndpi_struct, NDPI_LOG_DEBUG, "MMS avoid early exclude from http\n");
  }
#endif							/* NDPI_OLD_RESULT_BASE_HTTP */
