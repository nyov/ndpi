...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_DCERPC, NDPI_REAL_PROTOCOL);
}
...
void ndpi_search_dcerpc(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if((packet->tcp != NULL) 
    ...
    NDPI_LOG(NDPI_OLD_RESULT_APP_DCERPC, ndpi_struct, NDPI_LOG_DEBUG, "DCERPC match\n");	  
    ndpi_int_dcerpc_add_connection(ndpi_struct, flow);
    return;
  }

  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_DCERPC);
}
