...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_MAPLESTORY, protocol_type);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len == 16
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_MAPLESTORY, ndpi_struct, NDPI_LOG_DEBUG, "found maplestory.\n");
	...
	if (packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /maple")
		...
		if (packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /maple/patch")
			...
			if (packet->user_agent_line.ptr != NULL && packet->host_line.ptr != NULL
				...
				NDPI_LOG(NDPI_OLD_RESULT_APP_MAPLESTORY, ndpi_struct, NDPI_LOG_DEBUG, "found maplestory update.\n");
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_MAPLESTORY, ndpi_struct, NDPI_LOG_DEBUG, "exclude maplestory.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_MAPLESTORY);
