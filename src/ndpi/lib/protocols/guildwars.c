...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_GUILDWARS, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_GUILDWARS, ndpi_struct, NDPI_LOG_DEBUG, "search guildwars.\n");

	if (packet->payload_packet_len == 64 && get_u_int16_t(packet->payload, 1) == ntohs(0x050c)
		&& memcmp(&packet->payload[50], "@2&P", 4) == 0) {
		NDPI_LOG(NDPI_OLD_RESULT_APP_GUILDWARS, ndpi_struct, NDPI_LOG_DEBUG, "GuildWars version 29.350: found.\n");
	...
	if (packet->payload_packet_len == 21 && get_u_int16_t(packet->payload, 0) == ntohs(0x0100)
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_GUILDWARS, ndpi_struct, NDPI_LOG_DEBUG, "GuildWars version 216.107.245.50: found.\n");
		ndpi_int_guildwars_add_connection(ndpi_struct, flow);
		return;
	}

	NDPI_LOG(NDPI_OLD_RESULT_APP_GUILDWARS, ndpi_struct, NDPI_LOG_DEBUG, "exclude guildwars.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_GUILDWARS);
}
