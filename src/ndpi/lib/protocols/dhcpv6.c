...
#ifdef NDPI_OLD_RESULT_APP_DHCPV6

static void ndpi_int_dhcpv6_add_connection(struct ndpi_detection_module_struct *ndpi_struct,
					   struct ndpi_flow_struct *flow)
{

	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_DHCPV6, NDPI_REAL_PROTOCOL);
}
...
//  struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len >= 4 &&
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_DHCPV6, ndpi_struct, NDPI_LOG_DEBUG, "DHCPv6 detected.\n");
		ndpi_int_dhcpv6_add_connection(ndpi_struct, flow);
		return;
	}

	NDPI_LOG(NDPI_OLD_RESULT_APP_DHCPV6, ndpi_struct, NDPI_LOG_DEBUG, "DHCPv6 excluded.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_DHCPV6);
}
