											   ...
											   *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_SOULSEEK, NDPI_REAL_PROTOCOL);
								...
								*ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "Soulseek: search soulseec tcp \n");


	if (packet->detected_protocol_stack[0] == NDPI_OLD_RESULT_APP_SOULSEEK) {
		NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "packet marked as Soulseek\n");
		if (src != NULL)
			NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG,
					"  SRC bitmask: %u, packet tick %llu , last safe access timestamp: %llu\n",
					NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_OLD_RESULT_APP_SOULSEEK)
		...
		if (dst != NULL)
			NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG,
					...
					NDPI_COMPARE_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask, NDPI_OLD_RESULT_APP_SOULSEEK)
	...
	if (flow->l4.tcp.soulseek_stage == 0) {
				...
				if ((index + 4) <= packet->payload_packet_len && !get_u_int16_t(packet->payload, index + 2))	// for passwd len
				{
					...
					if ((index + 4 + 4) <= packet->payload_packet_len && !get_u_int16_t(packet->payload, index + 6))	// to read version,hashlen
					{
						...
						if (index == get_l32(packet->payload, 0)) {
							NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK,
		...
		if (packet->payload_packet_len > 8
			...
			if (msgcode == 0x7d) {
				...
				NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "Soulseek Messages Search\n");
				return;
				...
				if (src != NULL) {
					...
					if (packet->tcp != NULL && src->soulseek_listen_port == 0) {
						...
						NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct,
			...
			if (get_l32(packet->payload, 0) == packet->payload_packet_len - 4) {
				...
				if (packet->payload[4] == 0x01 && typelen == 1
					...
					NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "soulseek detected\n");
					ndpi_int_soulseek_add_connection(ndpi_struct, flow);
					return;
				}
				NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "1\n");
			}
			NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "3\n");
			...
			if (packet->payload_packet_len == 9 && get_l32(packet->payload, 0) == 5
				...
				flow->l4.tcp.soulseek_stage = 1 + packet->packet_direction;
				NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_TRACE, "Soulseek Size 9 Pierce Firewall\n");
				return;
			}
		...
		if (packet->payload_packet_len > 8) {
			if ((packet->payload[0] || packet->payload[1]) && get_l32(packet->payload, 4) == 9) {
				/* 9 is search result */
				NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "soulseek detected Second Pkt\n");
				ndpi_int_soulseek_add_connection(ndpi_struct, flow);
				return;
			}
			if (get_l32(packet->payload, 0) == packet->payload_packet_len - 4) {
	...
	if (flow->l4.tcp.soulseek_stage == 3 && packet->payload_packet_len == 8 && !get_u_int32_t(packet->payload, 4)) {
		...
		NDPI_LOG(NDPI_OLD_RESULT_APP_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG, "soulseek detected bcz of 8B  pkt\n");
		ndpi_int_soulseek_add_connection(ndpi_struct, flow);
		return;
	}
	if (flow->l4.tcp.soulseek_stage && flow->packet_counter < 11) {
	} else {
		NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_SOULSEEK);
	}
}
