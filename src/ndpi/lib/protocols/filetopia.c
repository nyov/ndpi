...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_OLD_RESULT_APP_FILETOPIA, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (flow->l4.tcp.filetopia_stage == 0) {
		if (packet->payload_packet_len >= 50 && packet->payload_packet_len <= 70
			...
			NDPI_LOG(NDPI_OLD_RESULT_APP_FILETOPIA, ndpi_struct, NDPI_LOG_DEBUG, "Filetopia stage 1 detected\n");
		...
		if (packet->payload_packet_len >= 100 && packet->payload[0] == 0x03
			...
			NDPI_LOG(NDPI_OLD_RESULT_APP_FILETOPIA, ndpi_struct, NDPI_LOG_DEBUG, "Filetopia stage 2 detected\n");
		...
		if (packet->payload_packet_len >= 4 && packet->payload_packet_len <= 100
			...
			NDPI_LOG(NDPI_OLD_RESULT_APP_FILETOPIA, ndpi_struct, NDPI_LOG_DEBUG, "Filetopia detected\n");
  ...
  end_filetopia_nothing_found:
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_OLD_RESULT_APP_FILETOPIA);
}
