...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_SOPCAST, NDPI_REAL_PROTOCOL);
}
									  ...
									  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (flow->packet_counter == 1 && packet->payload_packet_len == 54 && get_u_int16_t(packet->payload, 0) == ntohs(0x0036)) {
		if (ndpi_int_is_sopcast_tcp(packet->payload, packet->payload_packet_len)) {
			NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast TCP \n");
			ndpi_int_sopcast_add_connection(ndpi_struct, flow);
			return;
		}
	}

	NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "exclude sopcast TCP.  \n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_SOPCAST);
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "search sopcast.  \n");
	...
	if (packet->payload_packet_len == 52 && packet->payload[0] == 0xff
		...
		NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast with if I.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	if ((packet->payload_packet_len == 80 || packet->payload_packet_len == 28 || packet->payload_packet_len == 94)
		...
		NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast with if II.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	...
	if (packet->payload_packet_len == 60 && packet->payload[0] == 0x00
		...
		&& packet->payload[12] == 0x00 && packet->payload[13] == 0x00 && packet->payload[14] == 0x00) {
		NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast with if III.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	if (packet->payload_packet_len == 42 && packet->payload[0] == 0x00
		...
		NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast with if IV.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	if (packet->payload_packet_len == 28 && packet->payload[0] == 0x00
		...
		NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast with if V.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	/* this case has been seen once. Please revome this comment, if you see it another time */
	if (packet->payload_packet_len == 286 && packet->payload[0] == 0x00
		...
		NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast with if VI.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	if (packet->payload_packet_len == 76 && packet->payload[0] == 0xff
		...
		&& packet->payload[12] == 0x00 && packet->payload[13] == 0x00 && packet->payload[14] == 0x00) {
		NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "found sopcast with if VII.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	...
	NDPI_LOG(NDPI_RESULT_APP_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG, "exclude sopcast.  \n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_SOPCAST);
