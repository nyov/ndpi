...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_RTCP, NDPI_CORRELATED_PROTOCOL);
}
...
void ndpi_search_rtcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_RESULT_APP_RTCP, ndpi_struct, NDPI_LOG_DEBUG, "search for RTCP.\n");

  if(packet->tcp != NULL) {
    sport = ntohs(packet->tcp->source), dport = ntohs(packet->tcp->dest);
    NDPI_LOG(NDPI_RESULT_APP_RTCP, ndpi_struct, NDPI_LOG_DEBUG, "calculating dport over tcp.\n");
    ...
    if(packet->payload_packet_len > 13 && (sport == 554 || dport == 554) &&
      ...
      NDPI_LOG(NDPI_RESULT_APP_RTCP, ndpi_struct, NDPI_LOG_DEBUG, "found rtcp.\n");
      ndpi_int_rtcp_add_connection(ndpi_struct, flow);
    }
  } else if(packet->udp != NULL) {
    sport = ntohs(packet->udp->source), dport = ntohs(packet->udp->dest);
    NDPI_LOG(NDPI_RESULT_APP_RTCP, ndpi_struct, NDPI_LOG_DEBUG, "calculating dport over udp.\n");
    ...
    NDPI_LOG(NDPI_RESULT_APP_RTCP, ndpi_struct, NDPI_LOG_DEBUG, "exclude RTCP.\n");
    NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_RTCP);
  }
}
#endif
