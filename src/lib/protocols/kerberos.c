...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_KERBEROS, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len >= 4 && ntohl(get_u_int32_t(packet->payload, 0)) == packet->payload_packet_len - 4) {
		if (packet->payload_packet_len > 19 &&
			...
			NDPI_LOG(NDPI_RESULT_APP_KERBEROS, ndpi_struct, NDPI_LOG_DEBUG, "found KERBEROS\n");
	...
	NDPI_LOG(NDPI_RESULT_APP_KERBEROS, ndpi_struct, NDPI_LOG_DEBUG, "no KERBEROS detected.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_KERBEROS);
}
