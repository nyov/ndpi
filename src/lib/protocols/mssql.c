...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_MSSQL, NDPI_REAL_PROTOCOL);
}
						 ...
						 *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_RESULT_APP_MSSQL, ndpi_struct, NDPI_LOG_DEBUG, "search mssql.\n");
	...
	if (packet->payload_packet_len > 51 && ntohs(get_u_int32_t(packet->payload, 0)) == 0x1201
		...
		NDPI_LOG(NDPI_RESULT_APP_MSSQL, ndpi_struct, NDPI_LOG_DEBUG, "found mssql.\n");
		ndpi_int_mssql_add_connection(ndpi_struct, flow);
		return;
	}


	NDPI_LOG(NDPI_RESULT_APP_MSSQL, ndpi_struct, NDPI_LOG_DEBUG, "exclude mssql.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_MSSQL);
}
#endif
