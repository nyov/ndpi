...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_DHCP, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len >= 244 && (packet->udp->source == htons(67)
		...
		NDPI_LOG(NDPI_RESULT_APP_DHCP, ndpi_struct, NDPI_LOG_DEBUG, "DHCP request\n");

		ndpi_int_dhcp_add_connection(ndpi_struct, flow);
		return;
	}

	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_DHCP);
}
#endif
