...
static u16 parse_binf_message(struct ndpi_detection_module_struct
							  *ndpi_struct, const u8 * payload, int payload_len)
{
	u32 i = 4;
	u16 bytes_read = 0;
	u16 ssl_port = 0;
...
static void ndpi_int_directconnect_add_connection(struct ndpi_detection_module_struct
													*ndpi_struct, const u8 connection_type)
{
	...
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_DIRECTCONNECT, NDPI_REAL_PROTOCOL);
...
static void ndpi_search_directconnect_tcp(struct ndpi_detection_module_struct
											*ndpi_struct)
{
	struct ndpi_packet_struct *packet = &ndpi_struct->packet;
	struct ndpi_flow_struct *flow = ndpi_struct->flow;
	...
	if (src != NULL) {
		if (src->detected_directconnect_port == packet->tcp->source) {
			if ((NDPI_TIMESTAMP_COUNTER_SIZE)
				...
				ndpi_int_change_protocol(ndpi_struct, NDPI_PROTOCOL_DIRECTCONNECT, NDPI_REAL_PROTOCOL);
				src->directconnect_last_safe_access_time = packet->tick_timestamp;
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT, ndpi_struct,
	...
	if (flow->directconnect_stage == 0) {
		...
		if (packet->payload_packet_len >= 11) {
			...
			if (memcmp(&packet->payload[0], "HSUP ADBAS0", 11) == 0
				...
				ndpi_int_directconnect_add_connection(ndpi_struct, DIRECT_CONNECT_TYPE_HUB);
				return;
				/* did not see this pattern in any trace */
				...
				ndpi_int_directconnect_add_connection(ndpi_struct, DIRECT_CONNECT_ADC_PEER);
				return;
		...
		if (packet->payload_packet_len > 6) {
			if (packet->payload[0] == '$' && packet->payload[packet->payload_packet_len - 1] == '|') {
				...
				ndpi_int_directconnect_add_connection(ndpi_struct, DIRECT_CONNECT_TYPE_PEER);
								 ...
								 *ndpi_struct)
{
	...
	if (packet->tcp != NULL) {
		ndpi_search_directconnect_tcp(ndpi_struct);
	}
	if (packet->udp != NULL) {
		ndpi_search_directconnect_udp(ndpi_struct);
	}
}
