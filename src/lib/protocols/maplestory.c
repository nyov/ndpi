...
static void ndpi_int_maplestory_add_connection(struct ndpi_detection_module_struct
												 *ndpi_struct, ndpi_protocol_type_t protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_MAPLESTORY, protocol_type);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len == 16
		...
		ndpi_int_maplestory_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
		return;
	}
	...
	if (packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /maple")
		...
		if (packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /maple/patch")
			...
			if (packet->user_agent_line.ptr != NULL && packet->host_line.ptr != NULL
				...
				ndpi_int_maplestory_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
				return;
			}
