...
static void ndpi_int_msn_add_connection(struct ndpi_detection_module_struct
										  *ndpi_struct, ndpi_protocol_type_t protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_MSN, protocol_type);
}

static u8 ndpi_int_find_xmsn(struct ndpi_detection_module_struct *ndpi_struct)
{
...
static void ndpi_search_msn_tcp(struct ndpi_detection_module_struct *ndpi_struct)
{
	struct ndpi_packet_struct *packet = &ndpi_struct->packet;
	...
	if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_SSL) {
		...
		if (flow->packet_counter == 7 && packet->payload_packet_len > 300) {
			if (memcmp(packet->payload + 24, "MSNSLP", 6) == 0
				...
				ndpi_int_msn_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
				return;
			}
		}
	...
	if (flow->packet_counter == 1
		...
		if (packet->payload_packet_len > 32 && (packet->payload[0] == 0x02 || packet->payload[0] == 0x00)
			...
			if (packet->payload_packet_len > 101 && packet->payload[101] == 0x02) {
				ndpi_int_msn_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
			} else {
				ndpi_int_msn_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
			}
		...
		if (flow->packet_counter <= 10 &&
			...
			if (
				...
				if (ndpi_int_find_xmsn(ndpi_struct) == 1) {
		...
		if (packet->payload_packet_len >= 18) {
			...
			if (memcmp(packet->payload, "USR ", 4) == 0 || memcmp(packet->payload, "ANS ", 4) == 0) {
				...
				while (plen < endlen) {
					if (ndpi_check_for_email_address(ndpi_struct, plen) != 0) {
	...
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_MSN) == 0) {
		...
		if (packet->tcp != NULL) {
			...
			if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_UNKNOWN
				...
				ndpi_search_msn_tcp(ndpi_struct);
			}
		} else if (packet->udp != NULL) {
			ndpi_search_udp_msn_misc(ndpi_struct);
		}
	}
}
