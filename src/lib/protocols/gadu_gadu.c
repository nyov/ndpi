...
static void ndpi_int_gadugadu_add_connection(struct ndpi_detection_module_struct
											   *ndpi_struct, ndpi_protocol_type_t protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_GADUGADU, protocol_type);
}

static void parse_gg_foneno(struct ndpi_detection_module_struct *ndpi_struct)
{
...
static u8 check_for_gadugadu_payload_pattern(struct ndpi_detection_module_struct *ndpi_struct)
{
	struct ndpi_packet_struct *packet = &ndpi_struct->packet;
	...
	if (packet->payload_packet_len == 12) {
		...
		if ((flow->l4.tcp.gadugadu_stage == 1) && (flow->packet_counter == 2)
			...
			ndpi_int_gadugadu_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
			return 1;
		}
	}
	return 0;
...
static u8 check_for_http(struct ndpi_detection_module_struct *ndpi_struct)
{
	struct ndpi_packet_struct *packet = &ndpi_struct->packet;
	...
	if (packet->payload_packet_len < 50) {
		...
		ndpi_int_gadugadu_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
...
static void ndpi_search_gadugadu_tcp(struct ndpi_detection_module_struct
									   *ndpi_struct)
{
	struct ndpi_packet_struct *packet = &ndpi_struct->packet;
	struct ndpi_flow_struct *flow = ndpi_struct->flow;
...
/* here the asymmetric implementation ends */
	...
	if (packet->tcp != NULL && ((ntohs(packet->tcp->dest) == 80) || (ntohs(packet->tcp->source) == 80))) {
		if (check_for_gadugadu_payload_pattern(ndpi_struct)) {
			return;
		}
	}
...
void ndpi_search_gadugadu(struct ndpi_detection_module_struct *ndpi_struct)
{
	ndpi_search_gadugadu_tcp(ndpi_struct);
}
#endif
