...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_SNMP, NDPI_REAL_PROTOCOL);
}
	...
	if (packet->payload_packet_len > 32 && packet->payload[0] == 0x30) {
		...
		default:
			if (packet->payload[1] > 0x82) {
				NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP excluded, second byte is > 0x82\n");
				goto excl;
			}
			offset = 2;
		}
		...
		if (get_u_int16_t(packet->payload, offset) != htons(0x0201)) {
			NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP excluded, 0x0201 pattern not found\n");
			goto excl;
		}

		if (packet->payload[offset + 2] >= 0x04) {
			NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP excluded, version > 3\n");
			goto excl;
		}

		if (flow->l4.udp.snmp_stage == 0) {
			if (packet->udp->dest == htons(161) || packet->udp->dest == htons(162)) {
				NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP detected due to port.\n");
				ndpi_int_snmp_add_connection(ndpi_struct, flow);
				return;
			}
			NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP stage 0.\n");
			...
			if (packet->payload[offset + 2] == 0) {
				if (flow->l4.udp.snmp_msg_id != get_u_int8_t(packet->payload, offset + 15) - 1) {
					NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG,
							"SNMP v1 excluded, message ID doesn't match\n");
					goto excl;
				}
			}
		} else if (flow->l4.udp.snmp_stage == 2 - packet->packet_direction) {
			NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP stage 1-2.\n");
			if (packet->payload[offset + 2] == 3) {
			...
			NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP detected.\n");
			ndpi_int_snmp_add_connection(ndpi_struct, flow);
			return;
		}
	} else {
		NDPI_LOG(NDPI_RESULT_APP_SNMP, ndpi_struct, NDPI_LOG_DEBUG, "SNMP excluded.\n");
	}
  excl:
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_SNMP);
