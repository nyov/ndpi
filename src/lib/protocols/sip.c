			       ...
			       *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (payload_len > 4) {
    ...
    if (payload_len - 4 == message_len) {
      NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found STUN TURN ChannelData prefix.\n");
      payload_len -= 4;
      packet_payload += 4;
    }
  }
#ifndef NDPI_RESULT_APP_YAHOO
  if (payload_len >= 14 && packet_payload[payload_len - 2] == 0x0d && packet_payload[payload_len - 1] == 0x0a)
	...
	if ((memcmp(packet_payload, "NOTIFY ", 7) == 0 || memcmp(packet_payload, "notify ", 7) == 0)
	  ...
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip NOTIFY.\n");
	...
	if ((memcmp(packet_payload, "REGISTER ", 9) == 0 || memcmp(packet_payload, "register ", 9) == 0)
	  ...
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip REGISTER.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
	...
	if ((memcmp(packet_payload, "INVITE ", 7) == 0 || memcmp(packet_payload, "invite ", 7) == 0)
	    && (memcmp(&packet_payload[7], "SIP:", 4) == 0 || memcmp(&packet_payload[7], "sip:", 4) == 0)) {
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip INVITE.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
	...
	if (memcmp(packet_payload, "SIP/2.0 200 OK", 14) == 0 || memcmp(packet_payload, "sip/2.0 200 OK", 14) == 0) {
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip SIP/2.0 0K.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
        */
        if (memcmp(packet_payload, "SIP/2.0 ", 8) == 0 || memcmp(packet_payload, "sip/2.0 ", 8) == 0) {
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip SIP/2.0 *.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
	  ...
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip BYE.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
	    ...
	    && (memcmp(&packet_payload[4], "SIP:", 4) == 0 || memcmp(&packet_payload[4], "sip:", 4) == 0)) {
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip ACK.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
	  ...
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip CANCEL.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
	...
	if ((memcmp(packet_payload, "OPTIONS ", 8) == 0
		...
		|| memcmp(&packet_payload[8], "sip:", 4) == 0)) {
	  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "found sip OPTIONS.\n");
	  ndpi_int_sip_add_connection(ndpi_struct, flow, 0);
	  return;
	}
      }
  ...
  if (packet->udp != NULL && flow->packet_counter < 20) {
    NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "need next packet.\n");
    return;
  }
#ifdef NDPI_RESULT_APP_STUN
  /* for STUN flows we need some more packets */
  if (packet->udp != NULL && flow->detected_protocol_stack[0] == NDPI_RESULT_APP_STUN && flow->packet_counter < 40) {
    NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "need next STUN packet.\n");
    return;
  }
#endif

  if (payload_len == 4 && get_u_int32_t(packet_payload, 0) == 0) {
    NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "maybe sip. need next packet.\n");
    return;
  }
#ifdef NDPI_RESULT_APP_YAHOO
  if (payload_len > 30 && packet_payload[0] == 0x90
    ...
    NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "maybe sip yahoo. need next packet.\n");
  }
  if (flow->sip_yahoo_voice && flow->packet_counter < 10) {
    return;
  }
#endif
  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "exclude sip.\n");
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_SIP);
  return;
...
void ndpi_search_sip(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_RESULT_APP_SIP, ndpi_struct, NDPI_LOG_DEBUG, "sip detection...\n");

  /* skip marked packets */
  if (packet->detected_protocol_stack[0] != NDPI_RESULT_APP_SIP) {
