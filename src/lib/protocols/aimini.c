...
static void ndpi_int_aimini_add_connection(struct ndpi_detection_module_struct
											 *ndpi_struct, ndpi_protocol_type_t protocol_type)
{

	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_AIMINI, protocol_type);
}


static u8 is_special_aimini_host(struct ndpi_int_one_line_struct host_line)
{
...
void ndpi_search_aimini(struct ndpi_detection_module_struct *ndpi_struct)
{
	...
	if (packet->udp != NULL) {
		...
		if (flow->l4.udp.aimini_stage == 3
			...
			ndpi_int_aimini_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
			return;
		}
		...
		if ((packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /player/") &&
			...
			if (packet->host_line.ptr != NULL && packet->host_line.len > 11
				...
				ndpi_int_aimini_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
				return;
			}
		}
