...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_AIMINI, protocol_type);
}
...
void ndpi_search_aimini(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "search aimini.\n");
	...
	if (packet->udp != NULL) {
		if (flow->l4.udp.aimini_stage == 0) {
			if (packet->payload_packet_len == 64 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x010b) {
				...
				NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 1.\n");
				return;
			}
		...
		if (flow->l4.udp.aimini_stage == 1 && packet->payload_packet_len > 100
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 2.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 2 &&
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 3.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 3
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "found aimini (64, 0x010b), (>300, 0x0115), "
		...
		if (flow->l4.udp.aimini_stage == 4 && packet->payload_packet_len == 136
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 5.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 5 && (packet->payload_packet_len == 136
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 6.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 6 && ((packet->payload_packet_len == 136
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG,
		...
		if (flow->l4.udp.aimini_stage == 7 && packet->payload_packet_len == 88
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 8.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 8
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 9.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 10 && packet->payload_packet_len == 104
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 11.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 11
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 12.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 13 && packet->payload_packet_len == 32
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 14.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 14
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 15.\n");
			return;
		}
		...
		if (flow->l4.udp.aimini_stage == 16 && packet->payload_packet_len == 16
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 17.\n");
			return;
		}
		if (flow->l4.udp.aimini_stage == 17
			&& (packet->payload_packet_len == 16 && ntohs(get_u_int16_t(packet->payload, 0)) == 0x010c)) {
			flow->l4.udp.aimini_stage = 18;
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "stage = 18.\n");
			return;
		}
		...
		if ((packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /player/") &&
			...
			NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "HTTP packet detected.\n");
			...
			if (packet->host_line.ptr != NULL && packet->host_line.len > 11
				...
				NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "AIMINI HTTP traffic detected.\n");
	...
	NDPI_LOG(NDPI_RESULT_APP_AIMINI, ndpi_struct, NDPI_LOG_DEBUG, "exclude aimini.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_AIMINI);
