...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_WORLD_OF_KUNG_FU, NDPI_REAL_PROTOCOL);
}
...
void ndpi_search_world_of_kung_fu(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_RESULT_APP_WORLD_OF_KUNG_FU, ndpi_struct, NDPI_LOG_DEBUG, "search world_of_kung_fu.\n");
	...
	if ((packet->payload_packet_len == 16)
		...
		NDPI_LOG(NDPI_RESULT_APP_WORLD_OF_KUNG_FU, ndpi_struct, NDPI_LOG_DEBUG, "detected world_of_kung_fu.\n");
		ndpi_int_world_of_kung_fu_add_connection(ndpi_struct, flow);
		return;
	}

	NDPI_LOG(NDPI_RESULT_APP_WORLD_OF_KUNG_FU, ndpi_struct, NDPI_LOG_DEBUG, "exclude world_of_kung_fu.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_WORLD_OF_KUNG_FU);
}
