...
static void ndpi_int_zattoo_add_connection(struct ndpi_detection_module_struct
											 *ndpi_struct, ndpi_protocol_type_t protocol_type)
{
	...
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_ZATTOO, protocol_type);
...
#endif
	 u8 ndpi_int_zattoo_user_agent_set(struct ndpi_detection_module_struct *ndpi_struct)
{
	if (ndpi_struct->packet.user_agent_line.ptr != NULL && ndpi_struct->packet.user_agent_line.len == 111) {
		if (memcmp(ndpi_struct->packet.user_agent_line.ptr +
				   ndpi_struct->packet.user_agent_line.len - 25, "Zattoo/4", sizeof("Zattoo/4") - 1) == 0) {
...
void ndpi_search_zattoo(struct ndpi_detection_module_struct *ndpi_struct)
{
	struct ndpi_packet_struct *packet = &ndpi_struct->packet;
	...
	if (packet->tcp != NULL) {
		...
		if (packet->payload_packet_len > 20 && (packet->udp->dest == htons(5003)
			...
			if (++flow->zattoo_stage == 2) {
				...
				ndpi_int_zattoo_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
				return;
			}
