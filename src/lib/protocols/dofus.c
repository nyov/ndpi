...
static void ndpi_dofus_add_connection(struct ndpi_detection_module_struct
										*ndpi_struct)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_DOFUS, NDPI_REAL_PROTOCOL);
}

void ndpi_search_dofus(struct ndpi_detection_module_struct
						 *ndpi_struct)
{
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len == 13 && get_u16(packet->payload, 1) == ntohs(0x0508)
		...
		ndpi_dofus_add_connection(ndpi_struct);
		return;
	}
