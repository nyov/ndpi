...
static void ndpi_int_battlefield_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_BATTLEFIELD, NDPI_REAL_PROTOCOL);
...
void ndpi_search_battlefield(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->detected_protocol_stack[0] == NDPI_RESULT_APP_BATTLEFIELD) {
    if (src != NULL && ((u_int32_t)
			(packet->tick_timestamp - src->battlefield_ts) < ndpi_struct->battlefield_timeout)) {
      NDPI_LOG(NDPI_RESULT_APP_BATTLEFIELD, ndpi_struct, NDPI_LOG_DEBUG,
  ...
  if (NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_RESULT_APP_BATTLEFIELD)) {
	...
	NDPI_LOG(NDPI_RESULT_APP_BATTLEFIELD, ndpi_struct,
  ...
  if (packet->payload_packet_len == 18 && memcmp(&packet->payload[5], "battlefield2\x00", 13) == 0) {
    NDPI_LOG(NDPI_RESULT_APP_BATTLEFIELD, ndpi_struct, NDPI_LOG_DEBUG, "Battlefield 2 hello packet detected.\n");
    ndpi_int_battlefield_add_connection(ndpi_struct, flow);
    return;
    ...
    NDPI_LOG(NDPI_RESULT_APP_BATTLEFIELD, ndpi_struct, NDPI_LOG_DEBUG, "Battlefield safe pattern detected.\n");
    ndpi_int_battlefield_add_connection(ndpi_struct, flow);
    return;
  }

  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_BATTLEFIELD);
  return;
}
