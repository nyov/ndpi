...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_LDAP, NDPI_REAL_PROTOCOL);
}
...
//  u_int16_t dport;
	...
	NDPI_LOG(NDPI_RESULT_APP_LDAP, ndpi_struct, NDPI_LOG_DEBUG, "search ldap\n");
	...
	if (packet->payload_packet_len >= 14 && packet->payload[0] == 0x30) {
		...
		if (packet->payload[1] == 0x0c && packet->payload_packet_len == 14 &&
			...
			if (packet->payload[3] == 0x01 &&
				...
				NDPI_LOG(NDPI_RESULT_APP_LDAP, ndpi_struct, NDPI_LOG_DEBUG, "found ldap simple type 1\n");
			...
			if (packet->payload[3] == 0x02 &&
				...
				NDPI_LOG(NDPI_RESULT_APP_LDAP, ndpi_struct, NDPI_LOG_DEBUG, "found ldap simple type 2\n");
				ndpi_int_ldap_add_connection(ndpi_struct, flow);
				return;
			}
		}
		...
		if (packet->payload[1] == 0x84 && packet->payload_packet_len >= 0x84 &&
			...
			if (packet->payload[7] == 0x01 &&
				...
				NDPI_LOG(NDPI_RESULT_APP_LDAP, ndpi_struct, NDPI_LOG_DEBUG, "found ldap type 1\n");
				ndpi_int_ldap_add_connection(ndpi_struct, flow);
				return;
			}
			...
			if (packet->payload[7] == 0x02 &&
				...
				NDPI_LOG(NDPI_RESULT_APP_LDAP, ndpi_struct, NDPI_LOG_DEBUG, "found ldap type 2\n");
				ndpi_int_ldap_add_connection(ndpi_struct, flow);
				return;
			}
		}
	...
	NDPI_LOG(NDPI_RESULT_APP_LDAP, ndpi_struct, NDPI_LOG_DEBUG, "ldap excluded.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_LDAP);
}
