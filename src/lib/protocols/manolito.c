											   ...
											   *ndpi_struct)
{
	...
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_MANOLITO, NDPI_REAL_PROTOCOL);
...
*/
u8 search_manolito_tcp(struct ndpi_detection_module_struct *ndpi_struct);
u8 search_manolito_tcp(struct ndpi_detection_module_struct *ndpi_struct)
{
	struct ndpi_packet_struct *packet = &ndpi_struct->packet;
...
//  struct ndpi_id_struct *dst = ndpi_struct->dst;
  ...
  end_manolito_found:
	...
	ndpi_int_manolito_add_connection(ndpi_struct);
	return 1;
									...
									*ndpi_struct)
{
	...
	if (packet->tcp != NULL) {
		if (search_manolito_tcp(ndpi_struct) != 0)
			return;
