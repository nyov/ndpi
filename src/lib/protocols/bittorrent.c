...
#ifdef NDPI_DETECTION_SUPPORT_IPV6
    if(packet->iphv6)
      bt_add_announce(ndpi_struct->bt_ann, 1,
		      (ndpi_ip_addr_t *)&packet->iphv6->saddr,
		      s_port, &x.p,p_now);
#endif
    if(packet->iph)
      bt_add_announce(ndpi_struct->bt_ann, 0,
		      (ndpi_ip_addr_t *)&packet->iph->saddr,
		      s_port, &x.p,p_now);
  }
#endif
#ifdef NDPI_DETECTION_SUPPORT_IPV6
