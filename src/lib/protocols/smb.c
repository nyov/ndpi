...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_SMB, NDPI_REAL_PROTOCOL);
}
...
{  
	...
	if(packet && packet->tcp) {
	  NDPI_LOG(NDPI_RESULT_APP_SMB, ndpi_struct, NDPI_LOG_DEBUG, "search SMB.\n");
	  ...
	  if (packet->tcp->dest == htons(445)
	    ...
	    NDPI_LOG(NDPI_RESULT_APP_SMB, ndpi_struct, NDPI_LOG_DEBUG, "found SMB.\n");
	...
	NDPI_LOG(NDPI_RESULT_APP_SMB, ndpi_struct, NDPI_LOG_DEBUG, "exclude SMB.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_SMB);
}
