...
static void ndpi_int_smb_add_connection(struct ndpi_detection_module_struct
										  *ndpi_struct)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_SMB, NDPI_REAL_PROTOCOL);
}

void ndpi_search_smb_tcp(struct ndpi_detection_module_struct *ndpi_struct)
{
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->tcp->dest == htons(445)
		...
		ndpi_int_smb_add_connection(ndpi_struct);
		return;
