...
static void ndpi_int_jabber_add_connection(struct ndpi_detection_module_struct
											 *ndpi_struct, u32 protocol, ndpi_protocol_type_t protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, protocol, protocol_type);
}
		...
		if (packet->payload_packet_len > 47) {
			...
			for (x = 0; x < lastlen; x++) {
				if (ipq_mem_cmp
					...
					check_content_type_and_change_protocol(ndpi_struct, x);
