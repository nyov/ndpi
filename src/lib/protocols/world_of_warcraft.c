...
static void ndpi_int_worldofwarcraft_add_connection(struct
													  ndpi_detection_module_struct
													  *ndpi_struct, ndpi_protocol_type_t protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_WORLDOFWARCRAFT, protocol_type);
}
...
#endif
	 u8 ndpi_int_is_wow_port(const u16 port)
{
	...
	if (packet->tcp != NULL) {
		if ((packet->payload_packet_len > NDPI_STATICSTRING_LEN("POST /") &&
			...
			if (packet->user_agent_line.ptr != NULL &&
				...
				ndpi_int_worldofwarcraft_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
		...
		if (packet->payload_packet_len == 50 && memcmp(&packet->payload[2], "WORLD OF WARCRAFT CONNECTION",
			...
			ndpi_int_worldofwarcraft_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
