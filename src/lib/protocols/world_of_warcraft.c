...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_WORLDOFWARCRAFT, protocol_type);
}
				 ...
				 *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_RESULT_APP_WORLDOFWARCRAFT, ndpi_struct, NDPI_LOG_DEBUG, "Search World of Warcraft.\n");
  ...
  if (packet->tcp != NULL) {
	...
	NDPI_LOG(NDPI_RESULT_APP_WORLDOFWARCRAFT, ndpi_struct, NDPI_LOG_DEBUG,
    ...
    if (packet->payload_packet_len == 50 && memcmp(&packet->payload[2], "WORLD OF WARCRAFT CONNECTION",
      ...
      NDPI_LOG(NDPI_RESULT_APP_WORLDOFWARCRAFT, ndpi_struct, NDPI_LOG_DEBUG, "World of Warcraft: Login found\n");
      return;
    }
    ...
    if (NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_RESULT_APP_WORLDOFWARCRAFT) != 0) {
      if (packet->tcp->source == htons(3724)
	  && packet->payload_packet_len == 8 && get_u_int32_t(packet->payload, 0) == htonl(0x0006ec01)) {
	ndpi_int_worldofwarcraft_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	NDPI_LOG(NDPI_RESULT_APP_WORLDOFWARCRAFT, ndpi_struct,
	...
	NDPI_LOG(NDPI_RESULT_APP_WORLDOFWARCRAFT, ndpi_struct, NDPI_LOG_DEBUG, "waiting for final packet\n");
	return;
      }
    }
  ...
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_WORLDOFWARCRAFT);
}
