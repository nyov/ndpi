...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_IMESH, protocol_type);
}
...
void ndpi_search_imesh_tcp_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->udp != NULL) {
    ...
    NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "UDP FOUND\n");
	...
	(packet->udp->dest == htons(1864) || packet->udp->source == htons(1864))) {
      NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "iMesh Login detected\n");
	...
	NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "iMesh detected\n");
	ndpi_int_imesh_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	return;
      }
    ...
    NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "iMesh UDP packetlen: %d\n",
  ...
  if (packet->tcp != NULL) {
	...
	get_u_int16_t(packet->payload, 49) == htons(0x001a)) {
      NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "found imesh.\n");
      ndpi_int_imesh_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
    ...
    NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "TCP FOUND :: Payload %u\n",
	     packet->payload_packet_len);
	...
	&& get_u_int32_t(packet->payload, 4) == 0) {
      ...
      NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG,
  ...
  //imesh_not_found_end:
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_IMESH);
  NDPI_LOG(NDPI_RESULT_APP_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "iMesh excluded at stage %d\n",
