					      ...
					      struct ndpi_flow_struct *flow, ndpi_protocol_type_t protocol_type)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_CROSSFIRE, protocol_type);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_RESULT_APP_CROSSFIRE, ndpi_struct, NDPI_LOG_DEBUG, "search crossfire.\n");
	...
	if (packet->udp != 0) {
		if (packet->payload_packet_len == 25 && get_u_int32_t(packet->payload, 0) == ntohl(0xc7d91999)
			...
			NDPI_LOG(NDPI_RESULT_APP_CROSSFIRE, ndpi_struct, NDPI_LOG_DEBUG, "Crossfire: found udp packet.\n");
		...
		if (packet->payload_packet_len > 4 && memcmp(packet->payload, "GET /", 5) == 0) {
			...
			if (packet->parsed_lines == 8
				...
				NDPI_LOG(NDPI_RESULT_APP_CROSSFIRE, ndpi_struct, NDPI_LOG_DEBUG, "Crossfire: found HTTP request.\n");
	...
	NDPI_LOG(NDPI_RESULT_APP_CROSSFIRE, ndpi_struct, NDPI_LOG_DEBUG, "exclude crossfire.\n");
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_CROSSFIRE);
}
