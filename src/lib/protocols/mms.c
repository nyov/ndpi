...
static void ndpi_int_mms_add_connection(struct ndpi_detection_module_struct
										  *ndpi_struct)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_MMS, NDPI_REAL_PROTOCOL);
}


void ndpi_search_mms_tcp(struct ndpi_detection_module_struct *ndpi_struct)
{
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len >= 20) {
		...
		if (flow->l4.tcp.mms_stage == 2 - packet->packet_direction
			...
			ndpi_int_mms_add_connection(ndpi_struct);
			return;
		}
	}
