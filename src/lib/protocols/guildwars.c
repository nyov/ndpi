...
static void ndpi_int_guildwars_add_connection(struct ndpi_detection_module_struct
												*ndpi_struct)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_GUILDWARS, NDPI_REAL_PROTOCOL);
}

void ndpi_search_guildwars_tcp(struct ndpi_detection_module_struct
								 *ndpi_struct)
{
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len == 64 && get_u16(packet->payload, 1) == ntohs(0x050c)
		...
		ndpi_int_guildwars_add_connection(ndpi_struct);
		return;
	}
