...
static void ndpi_check_dropbox(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if(packet->udp != NULL) {
	...
	if(strncmp((const char *)packet->payload, "{\"", 2) == 0) {
	  NDPI_LOG(NDPI_RESULT_APP_DROPBOX, ndpi_struct, NDPI_LOG_DEBUG, "Found dropbox.\n");
  ...
  NDPI_LOG(NDPI_RESULT_APP_DROPBOX, ndpi_struct, NDPI_LOG_DEBUG, "exclude dropbox.\n");
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_DROPBOX);
}
...
void ndpi_search_dropbox(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_RESULT_APP_DROPBOX, ndpi_struct, NDPI_LOG_DEBUG, "dropbox detection...\n");

  /* skip marked packets */
  if (packet->detected_protocol_stack[0] != NDPI_RESULT_APP_DROPBOX) {
