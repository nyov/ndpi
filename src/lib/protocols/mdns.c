...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_MDNS, NDPI_REAL_PROTOCOL);
}
										 ...
										 *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if ((packet->payload[2] & 0x80) == 0 &&
		...
		NDPI_LOG(NDPI_RESULT_APP_MDNS, ndpi_struct, NDPI_LOG_DEBUG, "found MDNS with question query.\n");
		...
		NDPI_LOG(NDPI_RESULT_APP_MDNS, ndpi_struct, NDPI_LOG_DEBUG, "found MDNS with answer query.\n");
...
//      const u_int16_t sport=ntohs(packet->udp->source);
	...
	if (packet->udp != NULL) {
		...
		NDPI_LOG(NDPI_RESULT_APP_MDNS, ndpi_struct, NDPI_LOG_DEBUG, "MDNS udp start \n");
		...
		if (dport == 5353 && packet->payload_packet_len >= 12) {
			...
			NDPI_LOG(NDPI_RESULT_APP_MDNS, ndpi_struct, NDPI_LOG_DEBUG, "found MDNS with destination port 5353\n");
			...
			if (packet->iph != NULL && ntohl(packet->iph->daddr) == 0xe00000fb) {
				...
				NDPI_LOG(NDPI_RESULT_APP_MDNS, ndpi_struct,
	...
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_MDNS);
}
#endif
