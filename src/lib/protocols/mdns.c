...
static void ndpi_int_mdns_add_connection(struct ndpi_detection_module_struct
										   *ndpi_struct)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_MDNS, NDPI_REAL_PROTOCOL);
}
										 ...
										 *ndpi_struct)
{
	...
	if ((packet->payload[2] & 0x80) == 0 &&
		ntohs(get_u16(packet->payload, 4)) <= NDPI_MAX_MDNS_REQUESTS &&
		ntohs(get_u16(packet->payload, 6)) <= NDPI_MAX_MDNS_REQUESTS) {
	...
	if (packet->udp != NULL) {
		...
		if (dport == 5353 && packet->payload_packet_len >= 12) {
			...
			if (packet->iph != NULL && ntohl(packet->iph->daddr) == 0xe00000fb) {
				...
				if (ndpi_int_check_mdns_payload(ndpi_struct) == 1) {
					ndpi_int_mdns_add_connection(ndpi_struct);
					return;
				}
			}
