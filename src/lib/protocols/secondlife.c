...
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_SECONDLIFE, protocol_type);
}
...
//      ndpi_int_secondlife_add_connection(ndpi_struct);
//      return;
//  }
	...
	if (packet->tcp != NULL) {
		if (packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /")
			...
			if (packet->user_agent_line.ptr != NULL
				...
				ndpi_int_secondlife_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
				return;
			}
	...
	if (packet->udp != NULL) {
		if (packet->payload_packet_len == 46
			...
			ndpi_int_secondlife_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
			return;
		}
