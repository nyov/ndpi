...
static void ndpi_int_winmx_add_connection(struct ndpi_detection_module_struct
											*ndpi_struct, struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_WINMX, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (flow->l4.tcp.winmx_stage == 0) {
		...
		if (((packet->payload_packet_len) == 4)
			...
			NDPI_LOG(NDPI_RESULT_APP_WINMX, ndpi_struct, NDPI_LOG_DEBUG, "maybe WinMX Send\n");
		...
		if (((packet->payload_packet_len) == 3)
			...
			NDPI_LOG(NDPI_RESULT_APP_WINMX, ndpi_struct, NDPI_LOG_DEBUG, "found winmx by GET\n");
		...
		if (packet->payload_packet_len == 149 && packet->payload[0] == '8') {
			NDPI_LOG(NDPI_RESULT_APP_WINMX, ndpi_struct, NDPI_LOG_DEBUG, "maybe WinMX\n");
			if (get_u_int32_t(packet->payload, 17) == 0
				...
				NDPI_LOG(NDPI_RESULT_APP_WINMX, ndpi_struct, NDPI_LOG_DEBUG,
		...
		if (packet->payload_packet_len > 10 && packet->payload_packet_len < 1000) {
			...
			while (left > 0) {
				if (packet->payload[left] == ' ') {
					NDPI_LOG(NDPI_RESULT_APP_WINMX, ndpi_struct, NDPI_LOG_DEBUG, "found winmx in second packet\n");
					ndpi_int_winmx_add_connection(ndpi_struct, flow);
					return;
	...
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_WINMX);
}
