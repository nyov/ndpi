...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_SKINNY, NDPI_CORRELATED_PROTOCOL);
}
...
void ndpi_search_skinny(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG(NDPI_RESULT_APP_SKINNY, ndpi_struct, NDPI_LOG_DEBUG, "search for SKINNY.\n");

  if(packet->tcp != NULL) {
    sport = ntohs(packet->tcp->source), dport = ntohs(packet->tcp->dest);
    NDPI_LOG(NDPI_RESULT_APP_SKINNY, ndpi_struct, NDPI_LOG_DEBUG, "calculating SKINNY over tcp.\n");
    if (dport == 2000  && ((packet->payload_packet_len == 24 &&
      ...
      NDPI_LOG(NDPI_RESULT_APP_SKINNY, ndpi_struct, NDPI_LOG_DEBUG, "found skinny.\n");
    ...
    NDPI_LOG(NDPI_RESULT_APP_SKINNY, ndpi_struct, NDPI_LOG_DEBUG, "exclude SKINNY.\n");
    NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_SKINNY);
  }
}
#endif
