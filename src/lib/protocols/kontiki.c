...
static void ndpi_int_kontiki_add_connection(struct ndpi_detection_module_struct
											  *ndpi_struct)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_KONTIKI, NDPI_REAL_PROTOCOL);
}

void ndpi_search_kontiki(struct ndpi_detection_module_struct *ndpi_struct)
{
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len == 4 && (get_u32(packet->payload, 0) == htonl(0x02010100))) {
		...
		ndpi_int_kontiki_add_connection(ndpi_struct);
		return;
	}
