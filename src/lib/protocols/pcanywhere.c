...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_RESULT_APP_PCANYWHERE, NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->udp != NULL && packet->udp->dest == htons(5632)
		...
		NDPI_LOG(NDPI_RESULT_APP_PCANYWHERE, ndpi_struct, NDPI_LOG_DEBUG,
				"PC Anywhere name or status query detected.\n");
		ndpi_int_pcanywhere_add_connection(ndpi_struct, flow);
		return;
	}

	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_RESULT_APP_PCANYWHERE);
}
