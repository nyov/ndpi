...
static void ndpi_int_tvuplayer_add_connection(struct ndpi_detection_module_struct
												*ndpi_struct, ndpi_protocol_type_t protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, NDPI_PROTOCOL_TVUPLAYER, protocol_type);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->tcp != NULL) {
		if ((packet->payload_packet_len == 36 || packet->payload_packet_len == 24)
			...
			ndpi_int_tvuplayer_add_connection(ndpi_struct, NDPI_REAL_PROTOCOL);
			return;
		}
		...
		if (packet->payload_packet_len >= 50) {
			...
			if (memcmp(packet->payload, "POST", 4) || memcmp(packet->payload, "GET", 3)) {
				...
				if (packet->user_agent_line.ptr != NULL &&
					...
					ndpi_int_tvuplayer_add_connection(ndpi_struct, NDPI_CORRELATED_PROTOCOL);
					return;
				}
			}
		}
