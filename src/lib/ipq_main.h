...
typedef struct ndpi_packet_struct {
	...
	NDPI_TIMESTAMP_COUNTER_SIZE tick_timestamp;


	u16 detected_protocol_stack[NDPI_PROTOCOL_HISTORY_SIZE];
	u8 detected_subprotocol_stack[NDPI_PROTOCOL_HISTORY_SIZE];
...
typedef struct ndpi_detection_module_struct {
	...
	NDPI_TIMESTAMP_COUNTER_SIZE current_ts;
